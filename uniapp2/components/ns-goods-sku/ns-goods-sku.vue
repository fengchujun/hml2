		...
		<uni-popup ref="skuPopup" type="bottom" class="sku-layer" @change="popclose">
			<view class="sku-info" :style="{ height: skuHeight }">
					...
					<view class="main">
						<block v-if="type == 'point'">
							<block v-if="goodsDetail.type == 1">
								...
								<view class="stock" v-if="goodsDetail.stock_show">
						...
						<block v-else-if="type == 'presale'">
							<view class="price-wrap">
								...
								<text class="price price-style large">{{ parseFloat(goodsDetail.presale_deposit).toFixed(2).split('.')[0] }}</text>
								<text class="unit price-style small">.{{ parseFloat(goodsDetail.presale_deposit).toFixed(2).split('.')[1] }}</text>
							...
							<view class="stock presale-stock">
								<block v-if="goodsDetail.stock_show">库存{{ goodsDetail.stock }}{{ goodsDetail.unit }}
								</block>
							</view>
						</block>
						<block v-else>
							<view class="price-wrap" v-if="type == 'pintuan'">
								<block v-if="goodsDetail.pintuan_type == 'ordinary'">
									<block v-if="goodsDetail.group_id && goodsDetail.group_id > 0">
										<text class="unit price-style small">￥</text>
										<text class="price price-style large">{{ parseFloat(pintuanPrice(goodsDetail)).toFixed(2).split('.')[0] }}</text>
										<text class="unit price-style small">.{{ parseFloat(pintuanPrice(goodsDetail)).toFixed(2).split('.')[1] }}</text>
									</block>
									<block v-else>
										<text class="unit price-style small">￥</text>
										<text class="price price-style large">{{ parseFloat(goodsDetail.promotion_price).toFixed(2).split('.')[0] }}</text>
										<text class="unit price-style small">.{{ parseFloat(goodsDetail.promotion_price).toFixed(2).split('.')[1] }}</text>
									</block>
								</block>
							...
							<view class="price-wrap" v-else-if="type == 'groupbuy'">
								<text class="unit price-style small">￥</text>
								<text class="price price-style large">{{ parseFloat(goodsDetail.groupbuy_price).toFixed(2).split('.')[0] }}</text>
								<text class="unit price-style small">.{{ parseFloat(goodsDetail.groupbuy_price).toFixed(2).split('.')[1] }}</text>
							</view>
							<view class="price-wrap" v-else-if="type == 'topic'">
								<text class="unit price-style small">￥</text>
								<text class="price price-style large">{{ parseFloat(goodsDetail.topic_price).toFixed(2).split('.')[0] }}</text>
								<text class="unit price-style small">.{{ parseFloat(goodsDetail.topic_price).toFixed(2).split('.')[1] }}</text>
							</view>
							<view class="price-wrap" v-else-if="type == 'seckill'">
								<text class="unit price-style small">￥</text>
								<text class="price price-style large">{{ parseFloat(goodsDetail.seckill_price).toFixed(2).split('.')[0] }}</text>
								<text class="unit price-style small">.{{ parseFloat(goodsDetail.seckill_price).toFixed(2).split('.')[1] }}</text>
							</view>
				...
				<view class="body-item">
					<scroll-view scroll-y class="wrap">
						...
						<view class="sku-list-wrap" v-if="type == 'pintuan' && goodsDetail.pintuan_type == 'ladder' && !goodsDetail.group_id">
							...
							<view class="items" v-if="goodsDetail.pintuan_num_2 > 0" :class="{ selected: pintuan_num_field == 'pintuan_num_2' }" @click="pintuanChange('pintuan_num_2')">
								<text>{{ goodsDetail.pintuan_num_2 }}人团</text>
							</view>
							<view class="items" v-if="goodsDetail.pintuan_num_3 > 0" :class="{ selected: pintuan_num_field == 'pintuan_num_3' }" @click="pintuanChange('pintuan_num_3')">
						...
						<view class="number-wrap">
							<view class="number-line">
								...
								<view class="number">
									...
									<input type="number" class="uni-input color-line-border font-size-goods-tag" @blur="blur" v-model="number" placeholder="0" @input="keyInput(false)" />
				...
				<view class="footer" @click="confirm()">
					<button type="primary" v-if="goodsDetail.stock && goodsDetail.stock != 0 && goodsDetail.is_virtual == 0 && type == 'buy_now'" :style="{ background: themeStyle.goods_detail.goods_btn_color_shallow, color : '#fff' }" @click.stop="confirm(true)">加入购物车</button>
					<block v-if="type == 'point'">
						<block v-if="goodsDetail.type == 1">
							<!-- 兑换商品 -->
					...
					<block v-else-if="goodsDetail.goods_state == 1">
	...
	export default {
		...
		methods: {
			...
			getPointGoodsSkuList() {
				// 只有兑换商品才查询
				if (this.goodsDetail.type != 1) return;
				this.$api.sendRequest({
			...
			confirm(isJoinCartBtn = false) {
				...
				this.keyInput(true, () => {
					...
					if ((this.type == 'join_cart' || isJoinCartBtn) && this.goodsDetail.is_limit == 1 && this.maxBuy > 0 && this.cartNum + this.number > this.maxBuy) {
						this.$util.showToast({
...
<style lang="scss">
	...
	.sku-info .header .img-wrap image {
		width: inherit;
		height: inherit;
	}
	...
	.body-item .sku-list-wrap .selected {
		color: $base-color;
		border-color: $base-color;
	}

	.body-item .sku-list-wrap .items.disabled {
		border: 2rpx dashed;
	}

	.body-item .sku-list-wrap .items image {
		height: 44rpx;
		width: 44rpx;
		border-radius: $border-radius;
		margin-right: 10rpx;
	...
	.body-item .number-wrap .number-line {
		padding: 20rpx 0;
		line-height: 72rpx;
		display: flex;
	}

	.body-item .number-wrap .title {
		font-weight: 400;
	}

	.body-item .number-wrap .limit-txt {}

	.body-item .number-wrap .number {
		display: flex;
		height: 72rpx;
		border-radius: 6rpx;
		float: right;
	...
	.body-item .number-wrap .number button {
		display: inline-block;
		line-height: 64rpx;
		height: 68rpx;
		width: 60rpx;
	...
	.body-item .number-wrap .number button.decrease {
		border-right-width: 0 !important;
	}

	.body-item .number-wrap .number button.increase {
		border-left: 0 !important;
	}

	.body-item .number-wrap .number button:after {
		border-radius: 0;
		border: none;
	}

	.body-item .number-wrap .number input {
		display: inline-block;
		line-height: 64rpx;
		height: 68rpx;
		min-width: 90rpx;
