<template>
	...
	<view>
		...
		<!-- #endif -->
		<!-- #ifdef H5 -->
		<mescroll-uni ref="mescroll" @getData="getData" top="160rpx" >
		<!-- #endif -->
		<!-- #if !defined(MP) && !defined(H5) -->
		<mescroll-uni ref="mescroll" @getData="getData" top="80rpx" >
		<!-- #endif -->
			<block slot="list">
	...
	export default {
		...
		computed: {
			...
			currentCategoryName() {
				...
				const category = this.categoryList.find(item => item.category_id == this.categoryId);
				return category ? category.category_name : '文章列表';
			}
		},
		watch: {
			// 监听分类名称变化，动态设置导航栏标题
			currentCategoryName(newVal) {
				// #ifndef H5
				// 在小程序等非H5平台，设置原生导航栏标题
		...
		methods: {
			 ...
			 */
			scrollToCategory() {
				const index = this.categoryList.findIndex(item => item.category_id == this.categoryId);
				if (index !== -1) {
					this.scrollLeft = this.categoryList[index].left - this.contentScrollW / 2 + this.categoryList[index].width / 2;
				}
			},
			...
			updateNavigationTitle() {
				...
				// 在小程序等非H5平台，设置原生导航栏标题
				uni.setNavigationBarTitle({
					title: this.currentCategoryName
				});
				// #endif
			},
			getData(mescroll) {
