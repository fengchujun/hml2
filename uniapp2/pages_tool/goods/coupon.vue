			...
			<block slot="list">
				<view class="coupon-listone">
					<view class="item" v-for="(item, index) in list" :key="index" @click="liClick(item, index)" :style="{ backgroundColor: item.useState == 2 ? '#F2F2F2' : 'var(--main-color-shallow)' }">
						...
						<view class="item-btn">
							<view v-if="item.useState == 0" @click.stop="receiveCoupon(item, index)">去领取</view>
							...
							<view v-if="item.received_type == 'out'" class="disabled">已抢光</view>
							<view v-if="item.received_type == 'expire'" class="disabled">已过期</view>
