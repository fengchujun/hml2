...
class Goodscategory extends BaseApi
{
    ...
    public function tree()
    {
        ...
        $field = 'category_id,category_name,category_name_en,short_name,pid,level,image,image_en,category_id_1,category_id_2,category_id_3,image_adv,link_url,is_recommend,icon';
        $order = 'sort desc';
    ...
    public function info()
    {
        ...
        if (!empty($res[ 'data' ])) {
            ...
            ], 'category_id,category_name,category_name_en')[ 'data' ];
            $res[ 'data' ][ 'category_full_name' ] = implode('$_SPLIT_$', array_column($category_list, 'category_name'));

            $child_list = $goods_category_model->getCategoryList([ [ 'pid', '=', $category_id ], [ 'site_id', '=', $this->site_id ], [ 'is_show', '=', 0 ] ], 'category_id,category_name,category_name_en,short_name,pid,level,is_show,sort,image,image_en,attr_class_id,attr_class_name,category_id_1,category_id_2,category_id_3,commission_rate,image_adv,is_recommend,icon', 'sort asc,category_id desc')[ 'data' ];
            if (empty($child_list)) {
                // 查询上级商品分类
