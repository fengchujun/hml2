...
DROP TABLE IF EXISTS blindbox_category;

DROP TABLE IF EXISTS blindbox_goods;

DROP TABLE IF EXISTS blindbox_member_group;

DROP TABLE IF EXISTS blindbox_order;

DROP TABLE IF EXISTS cashier_auth;

DROP TABLE IF EXISTS cashier_auth_group;

DROP TABLE IF EXISTS cashier_pendorder;

DROP TABLE IF EXISTS cashier_pendorder_goods;

DROP TABLE IF EXISTS change_shifts_record;
...
DROP TABLE IF EXISTS diy_template_category;

DROP TABLE IF EXISTS diy_template_goods;

DROP TABLE IF EXISTS diy_template_goods_item;
...
DROP TABLE IF EXISTS express_deliver;
...
DROP TABLE IF EXISTS express_template_free_shipping;
...
DROP TABLE IF EXISTS goods_card_item;
...
DROP TABLE IF EXISTS member_goods_card;

DROP TABLE IF EXISTS member_goods_card_item;

DROP TABLE IF EXISTS member_goods_card_records;
...
DROP TABLE IF EXISTS service_position;
...
DROP TABLE IF EXISTS site_diy_template;
...
DROP TABLE IF EXISTS stat_store;

DROP TABLE IF EXISTS stat_store_hour;

DROP TABLE IF EXISTS stock_allot;

DROP TABLE IF EXISTS stock_allot_goods;

DROP TABLE IF EXISTS stock_content;

DROP TABLE IF EXISTS stock_document;

DROP TABLE IF EXISTS stock_document_goods;

DROP TABLE IF EXISTS stock_document_type;

DROP TABLE IF EXISTS stock_goods_export;

DROP TABLE IF EXISTS stock_inventory;

DROP TABLE IF EXISTS stock_inventory_goods;

DROP TABLE IF EXISTS `stock_transform`;

DROP TABLE IF EXISTS `stock_transform_goods`;
...
DROP TABLE IF EXISTS store_withdraw;

DROP TABLE IF EXISTS supplier;

DROP TABLE IF EXISTS sys_upgrade_log;

DROP TABLE IF EXISTS sys_patch;
...
DROP TABLE IF EXISTS `split_word`;
...
CREATE TABLE wechat_media (
  ...
  media_id varchar(70) NOT NULL DEFAULT '' COMMENT '微信端返回的素材id',
  PRIMARY KEY (id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
...
CREATE TABLE weapp_goods (
  ...
  status int(11) NOT NULL DEFAULT 0 COMMENT '商品状态，0：未审核。1：审核中，2：审核通过，3：审核驳回',
...
CREATE TABLE weapp_experiencer (
  id int(11) NOT NULL AUTO_INCREMENT,
  site_id int(11) NOT NULL DEFAULT 0 COMMENT '站点id',
  wechatid varchar(255) NOT NULL DEFAULT '' COMMENT '微信号',
  userstr varchar(255) NOT NULL DEFAULT '' COMMENT '微信用户唯一值',
...
CREATE TABLE verify_record (
  ...
  verify_time int(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
  verify_num int(11) NOT NULL DEFAULT 1 COMMENT '核销次数',
  verify_from varchar(255) NOT NULL DEFAULT 'mobile' COMMENT '核销端口',
  store_id int(11) NOT NULL DEFAULT 0 COMMENT '核销门店',
  PRIMARY KEY (id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
...
CREATE TABLE verify (
  ...
  expire_time int(11) NOT NULL DEFAULT 0 COMMENT '核销有效期 0永久',
  verify_from varchar(20) NOT NULL DEFAULT 'mobile' COMMENT '核销来源 shop 商家后台 store 门店后台 mobile 手机端',
  verify_remark varchar(50) NOT NULL DEFAULT '' COMMENT '核销备注',
  verify_total_count int(11) NOT NULL DEFAULT 0 COMMENT '核销次数 0为不限核销次数',
  verify_use_num int(11) NOT NULL DEFAULT 0 COMMENT '已核销次数',
  store_id int(11) NOT NULL DEFAULT 0 COMMENT '所属门店 0为全部门店可核销',
...
CREATE TABLE user_group (
  ...
  site_id int(11) NOT NULL DEFAULT 0 COMMENT '对应站id（门店，供应商）',
  store_id int(11) NOT NULL DEFAULT 0 COMMENT '管理的门店id',
  group_id int(11) NOT NULL DEFAULT 0 COMMENT '用户组id',
  create_time int(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
  app_module varchar(255) NOT NULL DEFAULT '',
  PRIMARY KEY (id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
...
ALTER TABLE `sys_upgrade_log` MODIFY COLUMN `error_message` varchar(3000) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT '' COMMENT '升级失败错误信息' AFTER `status`;

CREATE TABLE `sys_patch` (
    `patch_id` int NOT NULL DEFAULT '0',
    `patch_res` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL DEFAULT 'not' COMMENT '处理结果 not 未处理 complete 已处理 ignore 无需处理',
    `patch_time` int NOT NULL DEFAULT '0' COMMENT '处理时间',
    `patch_link` varchar(1000) COLLATE utf8mb4_general_ci NOT NULL DEFAULT '' COMMENT '补丁链接',
...
CREATE TABLE supplier (
  ...
  create_time int(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
  modify_time int(11) NOT NULL DEFAULT 0 COMMENT '修改时间',
  username varchar(255) NOT NULL DEFAULT '' COMMENT '供应商管理员',
...
CREATE TABLE store_withdraw (
  ...
  voucher_img varchar(255) NOT NULL DEFAULT '' COMMENT '凭证',
  voucher_desc varchar(255) NOT NULL DEFAULT '' COMMENT '凭证说明',
  fail_reason varchar(255) NOT NULL DEFAULT '' COMMENT '失败原因',
  settlement_type varchar(255) NOT NULL DEFAULT '' COMMENT '结算类型',
  settlement_type_name varchar(255) NOT NULL DEFAULT '' COMMENT '结算类型',
...
CREATE TABLE store_stock_import_log (
  ...
  stock DECIMAL(12, 3) NOT NULL DEFAULT 0.000 COMMENT '库存（增/减）',
...
CREATE TABLE store_stock_import (
  ...
  sku_num DECIMAL(12, 3) NOT NULL DEFAULT 0.000 COMMENT '导入的sku商品数',
  error_num DECIMAL(12, 3) NOT NULL DEFAULT 0.000 COMMENT '失败数量',
...
CREATE TABLE store_settlement (
  ...
  store_commission decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '门店抽成金额',
...
CREATE TABLE store_goods_sku (
  ...
  goods_id int(11) NOT NULL DEFAULT 0 COMMENT '商品id',
  stock DECIMAL(12, 3) NOT NULL DEFAULT 0.000 COMMENT '商品库存',
  sale_num DECIMAL(12, 3) NOT NULL DEFAULT 0.000 COMMENT '商品销量',
  store_id int(11) NOT NULL DEFAULT 0 COMMENT '门店id',
  create_time int(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
  modify_time int(11) NOT NULL DEFAULT 0 COMMENT '修改时间',
...
ALTER TABLE `store_goods_sku` ADD COLUMN `is_delivery_restrictions` int NOT NULL DEFAULT 1 COMMENT '是否有起送限制 1 有 0 没有' AFTER `real_stock`;
...
ALTER TABLE store_account ADD INDEX IDX_store_account_create_time (create_time);

ALTER TABLE store_account ADD INDEX IDX_store_account_from_type (from_type);

ALTER TABLE store_account ADD INDEX IDX_store_account_store_id (store_id);
...
CREATE TABLE store (
  ...
  o2o_fee_json varchar(1000) NOT NULL DEFAULT '' COMMENT '配送费用设置',
  create_time int(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
  modify_time int(11) NOT NULL DEFAULT 0 COMMENT '修改时间',
  ...
  stock_type varchar(255) NOT NULL DEFAULT 'all' COMMENT '库存设置 store-门店独立库存 all总部统一库存',
  is_default int(11) NOT NULL DEFAULT 0 COMMENT '是否是默认门店',
  store_type varchar(255) NOT NULL DEFAULT 'directsale' COMMENT '门店类型',
  ...
  account decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '当前账户余额',
  account_apply decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '转账申请中金额',
  account_withdraw decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '已结算转账金额',
  is_settlement tinyint(4) NOT NULL DEFAULT 0 COMMENT '是否需要结算',
  settlement_rate decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '开启结算0代表跟随系统，不为零表示独立设置',
  bank_type tinyint(4) NOT NULL DEFAULT 1 COMMENT '银行账户类型',
  bank_type_name varchar(255) NOT NULL DEFAULT '' COMMENT '账户名称',
  bank_user_name varchar(255) NOT NULL DEFAULT '' COMMENT '账户所属人',
  bank_type_account varchar(255) NOT NULL DEFAULT '' COMMENT '账户类型对应账号',
  category_id int(11) NOT NULL DEFAULT 0 COMMENT '门店分类id',
  category_name varchar(255) NOT NULL DEFAULT '' COMMENT '门店分类名称',
  label_id varchar(255) NOT NULL DEFAULT '' COMMENT '门店标签,id,id,',
  label_name varchar(5000) NOT NULL DEFAULT '' COMMENT '门店标签,name,name,',
  is_express int(11) NOT NULL DEFAULT 0 COMMENT '是否启用快递配送',
  store_images text DEFAULT NULL COMMENT '门店图片多图',
  store_introduce longtext DEFAULT NULL COMMENT '门店介绍',
...
ALTER TABLE `store` ADD COLUMN `open_date_config` varchar(1000) NOT NULL DEFAULT '[]' COMMENT '营业时间配置 json格式' AFTER `store_introduce`;

ALTER TABLE `store` ADD COLUMN `out_open_date_o2o_pay` tinyint(1) NOT NULL DEFAULT 1 COMMENT '非营业时间能否下同城配送单 1：能 0：不能' AFTER `open_date_config`;

ALTER TABLE `store` ADD COLUMN `close_show` tinyint(1) NOT NULL DEFAULT 0 COMMENT '门店休息是否前台展示' AFTER `out_open_date_o2o_pay`;

ALTER TABLE `store` ADD COLUMN `close_desc` varchar(255) NULL DEFAULT '' COMMENT '门店休息说明' AFTER `close_show`;

CREATE TABLE `stock_transform`  (
...
CREATE TABLE `stock_transform_goods`  (
    `transform_id` int NOT NULL DEFAULT 0 COMMENT '库存转换组id',
    `sku_id` int NOT NULL DEFAULT 0 COMMENT '规格id',
    `num` int NOT NULL DEFAULT 0 COMMENT '基本单位数量'
)
ENGINE = INNODB,
CHARACTER SET utf8,
COLLATE utf8_general_ci,
...
CREATE TABLE stock_inventory_goods (
  id int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
  inventory_id int(11) NOT NULL DEFAULT 0 COMMENT '盘点单',
  goods_id int(11) NOT NULL DEFAULT 0 COMMENT '商品id',
  goods_sku_id int(11) NOT NULL DEFAULT 0 COMMENT '商品sku',
  goods_sku_name varchar(255) NOT NULL DEFAULT '' COMMENT '商品名称',
  goods_sku_no varchar(255) NOT NULL DEFAULT '' COMMENT '商品编码',
  goods_img varchar(255) NOT NULL DEFAULT '' COMMENT '商品主图',
  stock DECIMAL(12, 3) NOT NULL DEFAULT 0.000 COMMENT '产品库数量',
  inventory_num DECIMAL(12, 3) NOT NULL DEFAULT 0.000 COMMENT '盘点数量',
  inventory_remark varchar(255) NOT NULL DEFAULT '' COMMENT '盘点备注',
  site_id int(11) NOT NULL DEFAULT 0,
  store_id int(11) NOT NULL DEFAULT 0 COMMENT '门店仓库id',
  profitloss_num DECIMAL(12, 3) NOT NULL DEFAULT 0.000 COMMENT '盈亏数量',
  inventory_cost_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '盘点成本总价',
  profitloss_sale_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '盈亏销售总价',
  goods_unit varchar(255) NOT NULL DEFAULT '' COMMENT '商品单位',
  goods_sku_spec varchar(255) NOT NULL DEFAULT '' COMMENT '产品规格',
  PRIMARY KEY (id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
...
ALTER TABLE `stock_inventory_goods` MODIFY COLUMN `inventory_cost_money` decimal(14, 2) NOT NULL DEFAULT 0.00 COMMENT '盘点成本总价' AFTER `profitloss_num`;
ALTER TABLE `stock_inventory_goods` MODIFY COLUMN `profitloss_sale_money` decimal(14, 2) NOT NULL DEFAULT 0.00 COMMENT '盈亏销售总价' AFTER `inventory_cost_money`;
...
CREATE TABLE stock_inventory (
  ...
  inventory_no varchar(255) NOT NULL DEFAULT '' COMMENT '盘点单号',
  store_id int(11) NOT NULL DEFAULT 0 COMMENT '仓库门店id',
  store_name varchar(255) NOT NULL DEFAULT '' COMMENT '仓库门店名称',
  kinds_num int(11) NOT NULL DEFAULT 0 COMMENT '总盘点种数',
  kinds_profit_num int(11) NOT NULL DEFAULT 0 COMMENT '盘盈种数',
  kinds_loss_num int(11) NOT NULL DEFAULT 0 COMMENT '盘亏种数',
  kinds_even_num int(11) NOT NULL DEFAULT 0 COMMENT '持平种数',
  num DECIMAL(12, 3) NOT NULL DEFAULT 0.000 COMMENT '盘点总数',
  profit_num DECIMAL(12, 3) NOT NULL DEFAULT 0.000 COMMENT '盘盈数',
  loss_num DECIMAL(12, 3) NOT NULL DEFAULT 0.000 COMMENT '盘亏数',
  even_num DECIMAL(12, 3) NOT NULL DEFAULT 0.000 COMMENT '盘点持平数',
  profitloss_num DECIMAL(12, 3) NOT NULL DEFAULT 0.000 COMMENT '盈亏数量',
  inventory_cost_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '盘点成本总额',
  profitloss_sale_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '盈亏销售额',
  status int(11) NOT NULL DEFAULT 0 COMMENT '审核状态 1：待审核，2：已审核，-1：已拒绝',
  operater int(11) NOT NULL DEFAULT 0 COMMENT '经办人id',
  operater_name varchar(255) NOT NULL DEFAULT '' COMMENT '经办人名称',
  verifier int(11) NOT NULL DEFAULT 0 COMMENT '审核人id',
...
ALTER TABLE `stock_inventory` MODIFY COLUMN `inventory_cost_money` decimal(14, 2) NOT NULL DEFAULT 0.00 COMMENT '盘点成本总额' AFTER `profitloss_num`;
ALTER TABLE `stock_inventory` MODIFY COLUMN `profitloss_sale_money` decimal(14, 2) NOT NULL DEFAULT 0.00 COMMENT '盈亏销售额' AFTER `inventory_cost_money`;

CREATE TABLE stock_goods_export (
...
CREATE TABLE stock_document_type (
  name varchar(50) NOT NULL DEFAULT '' COMMENT '单据名称',
  type varchar(255) NOT NULL DEFAULT '' COMMENT '单据类型output.出库input.入库',
  prefix varchar(255) NOT NULL DEFAULT '' COMMENT '单据前缀',
  `key` varchar(255) NOT NULL DEFAULT '' COMMENT '单据关键字'
)
ENGINE = INNODB,
CHARACTER SET utf8,
COLLATE utf8_general_ci,
...
CREATE TABLE stock_document_goods (
  document_goods_id int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
  document_id int(11) NOT NULL COMMENT '单据id',
  goods_id int(11) NOT NULL COMMENT '商品id',
  goods_sku_id int(11) NOT NULL COMMENT '商品skuid',
  goods_sku_no varchar(255) NOT NULL DEFAULT '' COMMENT '商品sku编码',
  goods_sku_img varchar(255) NOT NULL DEFAULT '' COMMENT '商品图片',
  goods_sku_name varchar(255) NOT NULL DEFAULT '' COMMENT '商品sku全称',
  goods_unit varchar(255) NOT NULL DEFAULT '' COMMENT '商品单位',
  goods_num DECIMAL(12, 3) NOT NULL DEFAULT 0.000 COMMENT '商品数量',
  goods_price decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '商品单价',
  goods_remark varchar(255) NOT NULL DEFAULT '' COMMENT '商品备注',
  create_time int(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
  site_id int(11) NOT NULL DEFAULT 0 COMMENT '站点id',
  before_stock DECIMAL(12, 3) NOT NULL DEFAULT 0.000 COMMENT '原库存',
  after_stock DECIMAL(12, 3) NOT NULL DEFAULT 0.000 COMMENT '现库存',
  before_goods_price decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '原成本价',
  after_goods_price decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '现成本均价',
  store_id int(11) NOT NULL DEFAULT 0 COMMENT '门店仓库id',
  operater int(11) NOT NULL DEFAULT 0 COMMENT '经办人id',
  operater_name varchar(255) NOT NULL DEFAULT '' COMMENT '经办人名称',
  verifier int(11) NOT NULL DEFAULT 0 COMMENT '审核人id',
  verifier_name varchar(255) NOT NULL DEFAULT '' COMMENT '审核人名称',
  before_store_stock DECIMAL(12, 3) NOT NULL DEFAULT 0.000,
  before_store_goods_price decimal(10, 2) NOT NULL DEFAULT 0.00,
  after_store_stock DECIMAL(12, 3) NOT NULL DEFAULT 0.000,
  after_store_goods_price decimal(10, 2) UNSIGNED NOT NULL DEFAULT 0.00,
  total_goods_money decimal(10, 2) NOT NULL DEFAULT 0.00,
  goods_sku_spec varchar(255) NOT NULL DEFAULT '' COMMENT '商品规格',
  PRIMARY KEY (document_goods_id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
COLLATE utf8_general_ci,
...
CREATE TABLE stock_document (
  ...
  document_no varchar(255) NOT NULL DEFAULT '' COMMENT '单据编号',
  `key` varchar(255) NOT NULL DEFAULT 'PURCHASE' COMMENT '单据关键字',
  type varchar(255) NOT NULL DEFAULT '' COMMENT '出入库类型 入库：input，出库：output',
  ...
  document_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '单据金额',
  remark varchar(255) NOT NULL DEFAULT '' COMMENT '备注',
  status int(11) NOT NULL DEFAULT 0 COMMENT '审核状态 1：待审核，2：已审核，-1：已拒绝',
  create_time int(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
  store_id int(11) NOT NULL DEFAULT 0 COMMENT '仓库id',
  store_name varchar(255) NOT NULL DEFAULT '' COMMENT '仓库门店名称',
  operater INT(11) NOT NULL COMMENT '经办人id',
  operater_name varchar(255) NOT NULL DEFAULT '' COMMENT '经办人名称',
  verifier int(11) NOT NULL DEFAULT 0 COMMENT '审核人id',
  verifier_name varchar(255) NOT NULL DEFAULT '' COMMENT '审核人名称',
  inventory_id int(11) NOT NULL DEFAULT 0 COMMENT '盘点单据id',
  time int(11) NOT NULL DEFAULT 0 COMMENT '出入库时间',
  is_out_stock int(11) NOT NULL DEFAULT 0 COMMENT '是否需要扣除销售库存',
  allot_id int(11) NOT NULL DEFAULT 0 COMMENT '调拨id',
...
ALTER TABLE `stock_document` MODIFY COLUMN `goods_money` decimal(14, 2) NOT NULL DEFAULT 0.00 COMMENT '商品金额' AFTER `type`;
ALTER TABLE `stock_document` MODIFY COLUMN `promotion_money` decimal(14, 2) NOT NULL DEFAULT 0.00 COMMENT '优惠金额' AFTER `goods_money`;
ALTER TABLE `stock_document` MODIFY COLUMN `invoice_money` decimal(14, 2) NOT NULL DEFAULT 0.00 COMMENT '发票金额' AFTER `promotion_money`;
ALTER TABLE `stock_document` MODIFY COLUMN `document_money` decimal(14, 2) NOT NULL DEFAULT 0.00 COMMENT '单据金额' AFTER `invoice_money`;
...
CREATE TABLE stock_allot_goods (
  allot_goods_id int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
  allot_id int(11) NOT NULL COMMENT '单据id',
  goods_id int(11) NOT NULL COMMENT '商品id',
  goods_sku_id int(11) NOT NULL COMMENT '商品skuid',
  goods_sku_no varchar(255) NOT NULL DEFAULT '' COMMENT '商品sku编码',
  goods_sku_img varchar(255) NOT NULL DEFAULT '' COMMENT '商品图片',
  goods_sku_name varchar(255) NOT NULL DEFAULT '' COMMENT '商品sku全称',
  goods_unit varchar(255) NOT NULL DEFAULT '' COMMENT '商品单位',
  goods_num DECIMAL(12, 3) NOT NULL DEFAULT 0.000 COMMENT '调拨商品数量',
  goods_price decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '商品单价',
  goods_remark varchar(255) NOT NULL DEFAULT '' COMMENT '商品备注',
  create_time int(11) NOT NULL DEFAULT 0,
  site_id int(11) NOT NULL DEFAULT 0 COMMENT '站点id',
  output_store_stock DECIMAL(12, 3) NOT NULL DEFAULT 0.000 COMMENT '出库门店原库存',
  input_store_stock DECIMAL(12, 3) NOT NULL DEFAULT 0.000 COMMENT '入库门店原库存',
  total_goods_money decimal(10, 2) NOT NULL DEFAULT 0.00,
  goods_sku_spec varchar(255) NOT NULL DEFAULT '' COMMENT '商品规格',
  PRIMARY KEY (allot_goods_id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
COLLATE utf8_general_ci,
...
CREATE TABLE stock_allot (
  allot_id int(11) NOT NULL AUTO_INCREMENT,
  site_id int(11) NOT NULL DEFAULT 0 COMMENT '站点',
  output_store_id int(11) NOT NULL COMMENT '出库门店',
  output_store_name varchar(255) NOT NULL DEFAULT '' COMMENT '出库门店',
  input_store_id int(11) NOT NULL COMMENT '入库门店',
  input_store_name varchar(255) NOT NULL DEFAULT '' COMMENT '入库门店',
  allot_no varchar(255) NOT NULL DEFAULT '' COMMENT '调拨编号',
  `key` varchar(255) NOT NULL DEFAULT 'PURCHASE' COMMENT '单据关键字',
  goods_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '商品金额',
  remark varchar(255) NOT NULL DEFAULT '' COMMENT '备注',
  status int(11) NOT NULL DEFAULT 0 COMMENT '审核状态 0草稿1.待审核2.已审核',
  verifier int(11) NOT NULL DEFAULT 0 COMMENT '审核人uid',
  verifier_name VARCHAR(255) NOT NULL DEFAULT '' COMMENT '审核人名称',
  operater INT(11) NOT NULL COMMENT '操作人id',
  operater_name varchar(255) NOT NULL DEFAULT '' COMMENT '操作人名称',
  allot_time int(11) NOT NULL DEFAULT 0 COMMENT '调拨时间',
  create_time int(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
...
CREATE TABLE stat_store_hour (
  ...
  billing_count int(11) NOT NULL DEFAULT 0 COMMENT '开单数量',
  billing_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '开单金额',
  buycard_count int(11) NOT NULL DEFAULT 0 COMMENT '办卡数量',
  buycard_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '办卡金额',
  recharge_count int(11) NOT NULL DEFAULT 0 COMMENT '充值数量',
  recharge_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '充值金额',
  refund_count int(11) NOT NULL DEFAULT 0 COMMENT '退款数量',
  refund_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '退款金额',
  order_member_count int(11) NOT NULL DEFAULT 0 COMMENT '下单会员数',
  balance_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '余额消费金额',
  online_pay_money DECIMAL(10, 2) NOT NULL DEFAULT 0.00 COMMENT '在线订单总额',
  online_refund_money DECIMAL(10, 2) NOT NULL DEFAULT 0.00 COMMENT '在线退款金额',
  balance_deduction DECIMAL(10, 2) NOT NULL DEFAULT 0.00 COMMENT '余额抵扣总额',
  PRIMARY KEY (id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
...
ALTER TABLE stat_store_hour ADD INDEX IDX_ns_stat_shop_day (day);

ALTER TABLE stat_store_hour ADD INDEX IDX_ns_stat_shop_day_time (day_time);

ALTER TABLE stat_store_hour ADD INDEX IDX_ns_stat_shop_month (month);

ALTER TABLE stat_store_hour ADD INDEX IDX_ns_stat_shop_site_id (site_id);

ALTER TABLE stat_store_hour ADD INDEX IDX_ns_stat_shop_year (year);
...
ALTER TABLE stat_store ADD INDEX IDX_ns_stat_shop_day (day);

ALTER TABLE stat_store ADD INDEX IDX_ns_stat_shop_day_time (day_time);

ALTER TABLE stat_store ADD INDEX IDX_ns_stat_shop_month (month);

ALTER TABLE stat_store ADD INDEX IDX_ns_stat_shop_site_id (site_id);

ALTER TABLE stat_store ADD INDEX IDX_ns_stat_shop_year (year);
...
CREATE TABLE stat_shop_hour (
  ...
  order_refund_grand_count decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '退款金额',
  order_refund_grand_total_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '累计退款金额',
  coupon_member_count int(11) NOT NULL DEFAULT 0 COMMENT '领券会员数量',
  member_level_count int(11) NOT NULL DEFAULT 0 COMMENT '超级会员卡销售量',
  member_level_total_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '超级会员卡销售额',
  member_level_grand_count int(11) NOT NULL DEFAULT 0 COMMENT '累计超级会员卡销售量',
  member_level_grand_total_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '累计超级会员卡销售额',
  member_recharge_count int(11) NOT NULL DEFAULT 0 COMMENT '会员储值总订单量',
  member_recharge_grand_count decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '累计会员储值总量',
  member_recharge_total_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '会员充值总额',
  member_recharge_grand_total_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '累计会员充值总额',
  member_recharge_member_count int(11) NOT NULL DEFAULT 0 COMMENT '储值会员数',
  member_giftcard_count int(11) NOT NULL DEFAULT 0 COMMENT '礼品卡订单总量',
  member_giftcard_grand_count int(11) NOT NULL DEFAULT 0 COMMENT '累计礼品卡订单总量',
  member_giftcard_total_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '礼品卡订单总额',
  h5_visit_count int(11) NOT NULL DEFAULT 0 COMMENT 'h5访问量',
  wechat_visit_count int(11) NOT NULL DEFAULT 0 COMMENT 'wechat访问量',
  weapp_visit_count int(11) NOT NULL DEFAULT 0 COMMENT 'weapp访问量',
  pc_visit_count int(11) NOT NULL DEFAULT 0 COMMENT 'pc访问量',
  expected_earnings_total_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '预计收入',
  expenditure_total_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '总支出',
  earnings_total_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '总收入',
  member_withdraw_count int(11) NOT NULL DEFAULT 0 COMMENT '会员提现总量',
  member_withdraw_total_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '会员提现总额',
  coupon_count int(11) NOT NULL DEFAULT 0 COMMENT '领券数',
  add_coupon_count int(11) NOT NULL DEFAULT 0 COMMENT '新增优惠券',
  order_pay_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '订单实际支付',
  add_fenxiao_member_count int(11) NOT NULL DEFAULT 0 COMMENT '新增分销商',
  fenxiao_order_total_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '分销订单总额',
  fenxiao_order_count int(11) NOT NULL DEFAULT 0 COMMENT '分销订单总数',
  goods_on_type_count int(11) NOT NULL DEFAULT 0 COMMENT '在架商品数',
  goods_visited_type_count int(11) NOT NULL DEFAULT 0 COMMENT '被访问商品数(仅详情页浏览数)',
  goods_order_type_count int(11) NOT NULL DEFAULT 0 COMMENT '动销商品数',
  goods_exposure_count int(11) NOT NULL DEFAULT 0 COMMENT '商品曝光数',
  goods_visit_count int(11) NOT NULL DEFAULT 0 COMMENT '商品浏览量',
  goods_visit_member_count int(11) NOT NULL DEFAULT 0 COMMENT '商品访客数',
  goods_cart_count int(11) NOT NULL DEFAULT 0 COMMENT '加购件数',
  goods_order_count DECIMAL(12, 3) NOT NULL DEFAULT 0.000 COMMENT '下单件数',
  order_create_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '订单下单总额',
  order_create_count int(11) NOT NULL DEFAULT 0 COMMENT '订单下单量',
  balance_deduction DECIMAL(10, 2) NOT NULL DEFAULT 0.00 COMMENT '余额抵扣总额',
  cashier_billing_count int(11) NOT NULL DEFAULT 0 COMMENT '开单数量',
  cashier_billing_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '开单金额',
  cashier_buycard_count int(11) NOT NULL DEFAULT 0 COMMENT '办卡数量',
  cashier_buycard_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '办卡金额',
  cashier_recharge_count int(11) NOT NULL DEFAULT 0 COMMENT '收银台充值数量',
  cashier_recharge_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '收银台充值金额',
  cashier_refund_count int(11) NOT NULL DEFAULT 0 COMMENT '收银台退款数量',
  cashier_refund_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '收银台退款金额',
  cashier_order_member_count int(11) NOT NULL DEFAULT 0 COMMENT '收银台下单会员数',
  cashier_balance_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '收银台余额消费金额',
  cashier_online_pay_money DECIMAL(10, 2) NOT NULL DEFAULT 0.00 COMMENT '收银台线上金额',
  cashier_online_refund_money DECIMAL(10, 2) NOT NULL DEFAULT 0.00 COMMENT '收银台线上退款金额',
  cashier_balance_deduction DECIMAL(10, 2) NOT NULL DEFAULT 0.00 COMMENT '门店余额总计',
  PRIMARY KEY (id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
...
CREATE TABLE stat_shop (
  ...
  order_create_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '订单下单总额',
  order_create_count int(11) NOT NULL DEFAULT 0 COMMENT '订单下单量',
  balance_deduction DECIMAL(10, 2) UNSIGNED NOT NULL DEFAULT 0.00 COMMENT '余额抵扣总额',
  cashier_billing_count int(11) NOT NULL DEFAULT 0 COMMENT '开单数量',
  cashier_billing_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '开单金额',
  cashier_buycard_count int(11) NOT NULL DEFAULT 0 COMMENT '办卡数量',
  cashier_buycard_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '办卡金额',
...
CREATE TABLE site_diy_view (
  id int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
  site_id int(11) NOT NULL DEFAULT 0 COMMENT '站点id',
  name varchar(50) NOT NULL DEFAULT '' COMMENT '模板标识',
  title varchar(255) NOT NULL DEFAULT '' COMMENT '模板名称',
  template_id int(11) NOT NULL DEFAULT 0 COMMENT '所属模板id',
  template_item_id int(11) NOT NULL DEFAULT 0 COMMENT '所属模板页面id，关联diy_template_goods_item表',
  type varchar(255) NOT NULL DEFAULT 'DIY_PAGE' COMMENT '页面类型',
  type_name varchar(255) NOT NULL DEFAULT '自定义页面' COMMENT '页面类型名称',
  is_default int(11) NOT NULL DEFAULT 0 COMMENT '是否默认页面（针对自定义模板设置），1：是，0：否',
  addon_name varchar(255) NOT NULL DEFAULT '' COMMENT '插件标识',
...
CREATE TABLE site_diy_template (
  id int(11) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键',
  name varchar(255) NOT NULL DEFAULT '' COMMENT '模板组名称',
  site_id int(11) NOT NULL DEFAULT 0 COMMENT '站点id',
  template_goods_id int(11) NOT NULL DEFAULT 0 COMMENT '模板组id',
  is_default int(11) NOT NULL DEFAULT 0 COMMENT '是否默认模板',
  addon_name varchar(255) NOT NULL DEFAULT '' COMMENT '插件标识',
  create_time int(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
  PRIMARY KEY (id)
)
...
CREATE TABLE site (
  ...
  site_tel varchar(255) NOT NULL DEFAULT '' COMMENT '服务电话',
  logo_square varchar(255) NOT NULL DEFAULT '' COMMENT '站点方形logo',
...
CREATE TABLE shop_accept_message (
  ...
  create_time int(11) NOT NULL DEFAULT 0,
  update_time int(11) NOT NULL DEFAULT 0,
  mobile varchar(50) NOT NULL DEFAULT '' COMMENT '通知人手机号',
  wx_openid varchar(255) NOT NULL DEFAULT '' COMMENT '通知人微信公众号openid',
  nickname varchar(255) NOT NULL DEFAULT '' COMMENT '通知人昵称',
  PRIMARY KEY (id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
...
CREATE TABLE shopcompoent_category_audit (
  ...
  site_id int(11) NOT NULL DEFAULT 0 COMMENT '站点ID',
  third_cat_id int(11) NOT NULL DEFAULT 0 COMMENT '第三级类目ID',
  certificate varchar(5000) NOT NULL DEFAULT '' COMMENT '类目资质材料',
...
CREATE TABLE shop (
  ...
  mobile varchar(255) NOT NULL DEFAULT '' COMMENT '联系手机号',
...
CREATE TABLE `service_member` (
  id INT(11) NOT NULL AUTO_INCREMENT,
  name VARCHAR(11) NOT NULL DEFAULT '' COMMENT '技师名称',
  headimg VARCHAR(11) NOT NULL DEFAULT '' COMMENT '头像',
  member_id INT(11) NOT NULL DEFAULT 0 COMMENT '关联会员',
  store_id INT(11) NOT NULL DEFAULT 0 COMMENT '关联门店',
  goods_ids VARCHAR(1000) NOT NULL DEFAULT '' COMMENT '关联服务',
  labels VARCHAR(1000) NOT NULL DEFAULT '' COMMENT '标签',
  status INT(11) NOT NULL DEFAULT 0 COMMENT '状态',
  ...
  depart_time INT(11) NOT NULL DEFAULT 0 COMMENT '离职时间',
  create_time INT(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
  introduce VARCHAR(2000) NOT NULL DEFAULT '' COMMENT '介绍',
  business_user INT(11) NOT NULL DEFAULT 0 COMMENT '业务人员',
  business_user_name VARCHAR(255) NOT NULL DEFAULT '' COMMENT '业务员',
  position_id INT(11) NOT NULL DEFAULT 0 COMMENT '岗位',
  position_name VARCHAR(255) NOT NULL DEFAULT '' COMMENT '岗位名称',
  PRIMARY KEY (id)
)
ENGINE = INNODB,
...
ALTER TABLE `service_member`
  ADD INDEX goods(goods_ids);

ALTER TABLE `service_member`
  ADD INDEX member(member_id);

ALTER TABLE `service_member`
  ADD INDEX store(store_id);
...
ALTER TABLE service_category ADD INDEX pid_level (pid, level);
...
ALTER TABLE `servicer_member` ADD INDEX `index_online`(`online`) USING BTREE COMMENT '是否在线';
...
ALTER TABLE `servicer_dialogue` ADD INDEX `index_unread_count`(`shop_id`, `read`, `member_id`) USING BTREE COMMENT '聊天未读消息';
...
CREATE TABLE scale (
  ...
  brand VARCHAR(255) NOT NULL DEFAULT '' COMMENT '电子秤品牌',
  brand_name VARCHAR(255) NOT NULL DEFAULT '' COMMENT '电子秤品牌名称',
  model VARCHAR(255) NOT NULL DEFAULT '' COMMENT '电子秤型号',
  model_name VARCHAR(255) NOT NULL DEFAULT '' COMMENT '型号名称',
  config TEXT DEFAULT NULL COMMENT '相关配置',
  status INT(11) NOT NULL DEFAULT 1 COMMENT '状态',
  create_time INT(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
  name VARCHAR(50) NOT NULL DEFAULT '' COMMENT '电子秤名称',
  type VARCHAR(255) NOT NULL DEFAULT 'barcode' COMMENT '秤类型 barcode 条码秤 cashier 收银秤',
  network_type VARCHAR(255) NOT NULL DEFAULT 'tcp' COMMENT '通信方式 serialport 串口 tcp tcp请求',
...
CREATE TABLE reserve_item (
  reserve_item_id int(11) NOT NULL AUTO_INCREMENT,
  reserve_id int(11) NOT NULL DEFAULT 0 COMMENT '预约id',
  site_id int(11) NOT NULL DEFAULT 0 COMMENT '站点',
  member_id int(11) NOT NULL DEFAULT 0 COMMENT '预约会员',
  ...
  reserve_user_id int(11) NOT NULL DEFAULT 0 COMMENT '预约服务人员id',
  reserve_goods_sku_id int(11) NOT NULL DEFAULT 0 COMMENT '预约服务id',
...
CREATE TABLE reserve (
  ...
  reserve_time int(11) NOT NULL DEFAULT 0 COMMENT '预约时间',
  reserve_state varchar(255) NOT NULL DEFAULT '' COMMENT '预约状态',
  reserve_state_name varchar(255) NOT NULL DEFAULT '',
  remark varchar(255) NOT NULL DEFAULT '' COMMENT '预约备注',
  store_id int(11) NOT NULL DEFAULT 0 COMMENT '门店id',
  create_time int(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
  complete_time int(11) NOT NULL DEFAULT 0 COMMENT '完成时间',
  to_store_time int(11) NOT NULL DEFAULT 0 COMMENT '到店时间',
  cancel_time int(11) NOT NULL DEFAULT 0 COMMENT '取消时间',
  source varchar(255) NOT NULL DEFAULT '' COMMENT '来源 store门店添加 member客户预约',
  reserve_item varchar(255) NOT NULL DEFAULT '',
...
CREATE TABLE promotion_seckill_time (
  ...
  site_id int(11) NOT NULL DEFAULT 0 COMMENT '站点id',
  name varchar(255) NOT NULL DEFAULT '' COMMENT '秒杀时段名称',
  create_time int(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
...
CREATE TABLE promotion_seckill (
  ...
  status tinyint(4) NOT NULL DEFAULT 0 COMMENT '活动状态 0未开始 1进行中 2已结束 -1已关闭（手动）',
  remark varchar(1000) NOT NULL DEFAULT '' COMMENT '活动规则说明',
...
CREATE TABLE promotion_presale_order (
  ...
  mobile varchar(255) NOT NULL DEFAULT '' COMMENT '购买人手机',
  telephone varchar(255) NOT NULL DEFAULT '' COMMENT '购买人固定电话',
  ...
  buyer_ask_delivery_time varchar(200) NOT NULL DEFAULT '' COMMENT '购买人要求配送时间',
...
CREATE TABLE promotion_pintuan (
  ...
  remark text DEFAULT NULL COMMENT '活动规则',
  create_time int(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
  modify_time int(11) NOT NULL DEFAULT 0 COMMENT '修改时间',
...
ALTER TABLE promotion_pinfan_order ADD INDEX IDX_ns_promotion_pintuan_order_head_id (head_id);

ALTER TABLE promotion_pinfan_order ADD INDEX IDX_ns_promotion_pintuan_order_member_id (member_id);

ALTER TABLE promotion_pinfan_order ADD INDEX IDX_ns_promotion_pintuan_order_order_id (order_id);

ALTER TABLE promotion_pinfan_order ADD INDEX IDX_ns_promotion_pintuan_order_pintuan_id (pintuan_id);

ALTER TABLE promotion_pinfan_order ADD INDEX IDX_ns_promotion_pintuan_order_site_id (site_id);
...
ALTER TABLE promotion_pinfan ADD INDEX IDX_ns_promotion_pintuan_end_time (end_time);

ALTER TABLE promotion_pinfan ADD INDEX IDX_ns_promotion_pintuan_goods_id (goods_id);

ALTER TABLE promotion_pinfan ADD INDEX IDX_ns_promotion_pintuan_is_recommend (is_recommend);

ALTER TABLE promotion_pinfan ADD INDEX IDX_ns_promotion_pintuan_site_id (site_id);

ALTER TABLE promotion_pinfan ADD INDEX IDX_ns_promotion_pintuan_start_time (start_time);

ALTER TABLE promotion_pinfan ADD INDEX IDX_ns_promotion_pintuan_status (status);
...
CREATE TABLE promotion_jielong_order (
  ...
  site_id int(11) NOT NULL DEFAULT 0 COMMENT '站点id',
  site_name varchar(255) NOT NULL DEFAULT '' COMMENT '站点名称',
  jielong_id int(11) NOT NULL DEFAULT 0 COMMENT '接龙活动id',
  order_no varchar(255) NOT NULL DEFAULT '' COMMENT '订单编号',
  order_from varchar(55) NOT NULL DEFAULT '' COMMENT '订单来源',
  order_from_name varchar(50) NOT NULL DEFAULT '' COMMENT '订单来源名称',
  order_type int(11) NOT NULL DEFAULT 0 COMMENT '订单类型 1. 普通订单  2. 门店订单  3. 本地配送订单4. 虚拟订单',
...
CREATE TABLE promotion_jielong (
  ...
  PRIMARY KEY (jielong_id)
);

ALTER TABLE `promotion_jielong` ADD COLUMN `take_start_time` int(11) NOT NULL DEFAULT 0 COMMENT '自提开始时间';
ALTER TABLE `promotion_jielong` ADD COLUMN `take_end_time` int(11) NOT NULL DEFAULT 0 COMMENT '自提结束时间';
...
CREATE TABLE promotion_hongbao_group (
  ...
  start_time int(11) NOT NULL DEFAULT 0 COMMENT '开始时间',
  end_time int(11) NOT NULL DEFAULT 0 COMMENT '红包Id',
  header_id int(11) NOT NULL DEFAULT 0 COMMENT '发起人Id',
  member_ids varchar(255) NOT NULL DEFAULT '' COMMENT '参与人id',
  balance_data varchar(255) NOT NULL DEFAULT '' COMMENT '瓜分的金额数（值与组Id对应）',
  group_member_ids varchar(255) NOT NULL DEFAULT '' COMMENT '组中人id（发放券的人）',
  num int(11) NOT NULL DEFAULT 0 COMMENT '参与数量',
  status int(11) NOT NULL DEFAULT 0 COMMENT '状态 0进行中 1瓜分成功 2瓜分失败',
  site_id int(11) NOT NULL DEFAULT 0,
  is_look int(11) NOT NULL DEFAULT 0 COMMENT '查看区分（瓜分失败后 0是重新组队 1是去查看）',
  PRIMARY KEY (group_id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
...
CREATE TABLE promotion_hongbao (
  ...
  image varchar(255) NOT NULL DEFAULT '' COMMENT '红包图片',
  start_time int(11) NOT NULL DEFAULT 0 COMMENT '活动开始时间',
  end_time int(11) NOT NULL DEFAULT 0 COMMENT '活动结束时间',
  status int(11) NOT NULL DEFAULT 0 COMMENT '状态 0未开始 1进行中 2已结束 -1已关闭',
  status_name varchar(64) NOT NULL DEFAULT '' COMMENT '状态名称',
  count int(11) NOT NULL DEFAULT 0 COMMENT '发放总量',
  inventory int(11) DEFAULT 0 COMMENT '红包库存',
  success_count int(11) NOT NULL DEFAULT 0 COMMENT '瓜分成功的红包数量',
  money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '红包瓜分金额',
  divide_num int(11) NOT NULL DEFAULT 0 COMMENT '瓜分人数',
  divide_type int(11) NOT NULL DEFAULT 0 COMMENT '瓜分方式（0固定金额 1随机金额）',
  is_simulation int(11) NOT NULL DEFAULT 0 COMMENT '是否模拟好友 0否 1是',
  remark varchar(255) NOT NULL DEFAULT '' COMMENT '活动规则说明',
  divide_time int(11) NOT NULL DEFAULT 1 COMMENT '瓜分有效期(小时)',
  is_new int(11) NOT NULL DEFAULT 0 COMMENT '仅新人参与限制 0否 1是',
  balance_set int(11) NOT NULL DEFAULT 1 COMMENT '余额设置  1不可提现余额 2可提现余额',
  create_time int(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
...
CREATE TABLE promotion_groupbuy (
  ...
  status tinyint(4) NOT NULL DEFAULT 1 COMMENT '状态（1未开始  2进行中  3已结束）',
  sku_id int(11) NOT NULL DEFAULT 0 COMMENT '商品sku',
  rule text DEFAULT NULL COMMENT '活动规则',
...
CREATE TABLE promotion_games (
  ...
  join_type int(11) NOT NULL DEFAULT 0 COMMENT '参加类型 0活动全过程 1每天 (节日礼 0节日前某天 1节日当天)',
  join_frequency int(11) NOT NULL DEFAULT 1 COMMENT '根据类型计算参加次数(节日礼的天数)',
  join_num int(11) NOT NULL DEFAULT 0 COMMENT '抽奖人数',
  winning_num int(11) NOT NULL DEFAULT 0 COMMENT '中奖人数',
  create_time int(11) NOT NULL DEFAULT 0,
  update_time int(11) NOT NULL DEFAULT 0,
  push_time int(11) NOT NULL DEFAULT 0 COMMENT '节日有礼活动推送时间戳',
  PRIMARY KEY (game_id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
...
CREATE TABLE promotion_friends_coupon_group (
  ...
  header_id int(11) NOT NULL DEFAULT 0 COMMENT '发起人Id',
  member_ids varchar(255) NOT NULL DEFAULT '' COMMENT '参与人id',
  coupon_ids varchar(255) NOT NULL DEFAULT '' COMMENT '优惠券组Id',
  group_member_ids varchar(255) NOT NULL DEFAULT '' COMMENT '组中人id（发放券的人）',
  num int(11) NOT NULL DEFAULT 0 COMMENT '参与数量',
  status int(11) NOT NULL DEFAULT 0 COMMENT '状态 0进行中 1瓜分成功 2瓜分失败',
  is_look int(11) NOT NULL DEFAULT 0 COMMENT '查看区分（瓜分失败后 0是重新组队 1是去查看）',
...
CREATE TABLE promotion_friends_coupon (
  ...
  coupon_type_id int(11) NOT NULL DEFAULT 0 COMMENT '优惠券类型Id',
  name varchar(64) NOT NULL DEFAULT '' COMMENT '活动名称',
  goods_type int(11) NOT NULL DEFAULT 1 COMMENT '适用商品类型1-全部商品可用；2-指定商品可用；3-指定商品不可用',
  goods_ids varchar(2500) NOT NULL DEFAULT '' COMMENT '商品Id',
  ...
  status_name varchar(64) NOT NULL DEFAULT '' COMMENT '状态名称',
  count int(11) NOT NULL DEFAULT 0 COMMENT '发放总量',
  inventory int(11) DEFAULT 0 COMMENT '优惠券库存',
  success_count int(11) NOT NULL DEFAULT 0 COMMENT '瓜分成功的优惠券数量',
  money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '优惠券瓜分金额',
  divide_num int(11) NOT NULL DEFAULT 0 COMMENT '瓜分人数',
  validity_type int(11) NOT NULL DEFAULT 1 COMMENT '过期类型0-固定时间1领取之日起',
  start_use_time int(11) NOT NULL DEFAULT 0 COMMENT '使用开始日期 过期类型1时必填',
  end_use_time int(11) NOT NULL DEFAULT 0 COMMENT '使用结束日期 过期类型1时必填',
  fixed_term int(11) NOT NULL DEFAULT 0 COMMENT '当validity_type为2或者3时需要添加 领取之日起或者次日N天内有效',
  divide_type int(11) NOT NULL DEFAULT 0 COMMENT '瓜分方式（0固定金额 1随机金额）',
  is_simulation int(11) NOT NULL DEFAULT 0 COMMENT '是否模拟好友 0否 1是',
  remark varchar(255) NOT NULL DEFAULT '' COMMENT '活动规则说明',
  divide_time int(11) NOT NULL DEFAULT 1 COMMENT '瓜分有效期(小时)',
  is_limit int(11) NOT NULL DEFAULT 0 COMMENT '使用门槛 0无门槛 1 有门槛',
  at_least decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '满多少元使用 0代表无限制',
  is_new int(11) NOT NULL DEFAULT 0 COMMENT '仅新人参与限制 0否 1是',
  validity_end_time int(11) NOT NULL DEFAULT 0 COMMENT '使用券有效期结束时间',
  create_time int(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
...
CREATE TABLE promotion_festival_draw_record (
  ...
  site_id int(11) NOT NULL DEFAULT 0,
  festival_id int(11) NOT NULL DEFAULT 0 COMMENT '活动id',
  festival_type varchar(30) NOT NULL DEFAULT '' COMMENT '活动类型（插件名称）',
  member_id int(11) NOT NULL DEFAULT 0 COMMENT '会员id',
  member_nick_name varchar(255) NOT NULL DEFAULT '' COMMENT '会员昵称',
  award_id int(11) NOT NULL DEFAULT 0 COMMENT '奖品id',
...
CREATE TABLE promotion_festival_award (
  ...
  site_id int(11) NOT NULL DEFAULT 0,
  festival_id int(11) NOT NULL DEFAULT 0,
...
CREATE TABLE promotion_festival (
  ...
  site_id int(11) NOT NULL DEFAULT 0,
  festival_name varchar(255) NOT NULL DEFAULT '' COMMENT '节日名',
  festival_type varchar(30) NOT NULL DEFAULT '1' COMMENT '活动类型（插件名称）',
  festival_type_name varchar(255) NOT NULL DEFAULT '' COMMENT '活动类型名称',
  level_id varchar(255) NOT NULL DEFAULT '' COMMENT '参与的会员等级0表示全部参与',
  level_name varchar(255) NOT NULL DEFAULT '' COMMENT '参与活动会员名称',
  start_time int(11) NOT NULL DEFAULT 0 COMMENT '活动开始时间',
  end_time int(11) NOT NULL DEFAULT 0 COMMENT '结束时间',
  status int(11) NOT NULL DEFAULT 0 COMMENT '活动状态 0未开始 1已开始 2已结束 3已关闭',
  push_time int(11) NOT NULL DEFAULT 0 COMMENT '活动推送时间',
...
CREATE TABLE promotion_exchange_order (
  ...
  delivery_store_id int(11) NOT NULL DEFAULT 0 COMMENT '门店id',
  delivery_store_name varchar(255) NOT NULL DEFAULT '' COMMENT '门店名称',
  delivery_store_info varchar(2000) NOT NULL DEFAULT '' COMMENT '门店信息',
...
CREATE TABLE promotion_discount_goods (
  ...
  sku_name varchar(255) NOT NULL DEFAULT '' COMMENT '商品名称',
  sku_image varchar(1000) NOT NULL DEFAULT '' COMMENT '商品图片',
  PRIMARY KEY (id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
...
CREATE TABLE promotion_coupon_type (
  ...
  min_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '最低金额 当type为random时需要添加',
  max_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '最大金额 当type为random时需要添加',
...
ALTER TABLE `promotion_coupon_type` ADD COLUMN `goods_names` varchar(3000) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT '' COMMENT '适用商品名称' AFTER `goods_ids`;

ALTER TABLE `promotion_coupon_type` MODIFY COLUMN `goods_ids` varchar(3000) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT '' COMMENT '适用商品id' AFTER `goods_type`;

ALTER TABLE `promotion_coupon_type` MODIFY COLUMN `count` int NOT NULL DEFAULT 0 COMMENT '可领取总数' AFTER `image`;

ALTER TABLE `promotion_coupon_type` ADD COLUMN `give_count` int NOT NULL DEFAULT 0 COMMENT '发放数量（营销活动发放和直接发放）' AFTER `used_count`;
...
CREATE TABLE promotion_consume_record (
  id int(11) NOT NULL AUTO_INCREMENT,
  site_id int(11) NOT NULL DEFAULT 0 COMMENT '站点id',
  type varchar(255) NOT NULL DEFAULT '' COMMENT '赠送类型 point-积分 growth-成长值 coupon-优惠券',
  value int(11) NOT NULL DEFAULT 0 COMMENT '所送内容',
  order_id int(11) NOT NULL DEFAULT 0 COMMENT '订单id',
  member_id int(11) NOT NULL DEFAULT 0,
...
CREATE TABLE promotion_birthdaygift_record (
  ...
  activity_id int(11) NOT NULL DEFAULT 0 COMMENT '生日有礼活动id',
  receive_time int(11) NOT NULL DEFAULT 0 COMMENT '领取时间',
  PRIMARY KEY (record_id)
)
ENGINE = INNODB,
...
CREATE TABLE printer_template (
  ...
  create_time int(11) NOT NULL DEFAULT 0,
  update_time int(11) NOT NULL DEFAULT 0,
  type varchar(50) NOT NULL DEFAULT '' COMMENT '模板类型',
  type_name varchar(50) NOT NULL DEFAULT '' COMMENT '模板类型名称',
  goods_price_show int(11) NOT NULL DEFAULT 0 COMMENT '商品金额是否展示',
  goods_code_show int(11) NOT NULL DEFAULT 1 COMMENT '商品编码是否展示',
  goods_price_type varchar(50) NOT NULL DEFAULT 'price' COMMENT '商品金额 price-售价，order_price-实付价',
  form_show int(11) NOT NULL DEFAULT 1 COMMENT '是否显示表单',
  PRIMARY KEY (template_id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
...
CREATE TABLE printer (
  ...
  create_time int(11) NOT NULL DEFAULT 0,
  update_time int(11) NOT NULL DEFAULT 0,
  manual_open int(11) NOT NULL DEFAULT 1 COMMENT '手动打印开启',
  order_pay_open int(11) NOT NULL DEFAULT 1 COMMENT '订单支付打印开启',
  order_pay_template_id int(11) NOT NULL DEFAULT 0 COMMENT '订单支付打印模板',
  order_pay_print_num int(11) NOT NULL DEFAULT 1 COMMENT '订单支付打印张数',
  order_pay_order_type varchar(255) NOT NULL DEFAULT '' COMMENT '订单支付打印订单类型',
  take_delivery_open int(11) NOT NULL DEFAULT 1 COMMENT '订单收货打印开启',
  take_delivery_template_id int(11) NOT NULL DEFAULT 0 COMMENT '订单收货打印模板',
  take_delivery_print_num int(11) NOT NULL DEFAULT 1 COMMENT '订单收货打印张数',
  take_delivery_order_type varchar(255) NOT NULL DEFAULT '' COMMENT '订单收货打印订单类型',
  recharge_open int(11) NOT NULL DEFAULT 1 COMMENT '充值打印开启',
  recharge_template_id int(11) NOT NULL DEFAULT 0 COMMENT '充值打印模板',
  recharge_print_num int(11) NOT NULL DEFAULT 1 COMMENT '充值打印张数',
  change_shifts_open int(11) NOT NULL DEFAULT 1 COMMENT '收银开单打印开启',
  change_shifts_template_id int(11) NOT NULL DEFAULT 0 COMMENT '收银开单打印模板',
  change_shifts_print_num int(11) NOT NULL DEFAULT 1 COMMENT '收银开单打印张数',
  printer_type varchar(10) NOT NULL DEFAULT 'cloud' COMMENT '打印机类型 cloud 云打印机 local 本地打印机 network 网络打印机',
  host varchar(255) NOT NULL DEFAULT '',
  ip varchar(255) NOT NULL DEFAULT '',
  port varchar(255) NOT NULL DEFAULT '',
  print_width VARCHAR(20) NOT NULL DEFAULT '58mm' COMMENT '打印宽度',
...
CREATE TABLE poster_template (
  template_id int(11) NOT NULL AUTO_INCREMENT,
  site_id int(11) NOT NULL DEFAULT 0 COMMENT '站点id',
  poster_name varchar(50) NOT NULL DEFAULT '' COMMENT '海报名称',
  background varchar(255) NOT NULL DEFAULT '' COMMENT '海报背景',
  qrcode_type varchar(255) NOT NULL DEFAULT 'qrcode' COMMENT '带参二维码，常规二维码',
...
CREATE TABLE poster_record (
  ...
  from_type varchar(255) NOT NULL DEFAULT '' COMMENT '来源类型，visit：访客，order：订单',
  year int(11) NOT NULL DEFAULT 0 COMMENT '年',
  month int(11) NOT NULL DEFAULT 0 COMMENT '月',
  day int(11) NOT NULL DEFAULT 0 COMMENT '日',
  create_time int(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
...
CREATE TABLE poster_muban (
  ...
  qrcode_height decimal(10, 0) NOT NULL DEFAULT 0,
  qrcode_top decimal(10, 0) NOT NULL DEFAULT 0,
  qrcode_left decimal(10, 0) NOT NULL DEFAULT 0,
  template_json text DEFAULT NULL,
  background varchar(255) NOT NULL DEFAULT '',
  template_type varchar(255) NOT NULL DEFAULT '' COMMENT '模板类型',
  qrcode_type varchar(255) NOT NULL DEFAULT '' COMMENT '二维码类型',
...
ALTER TABLE `poster` ADD COLUMN `param_md5` varchar(255) NOT NULL DEFAULT '' COMMENT '参数md5值' AFTER `create_time`;

ALTER TABLE `poster` ADD COLUMN `file_path` varchar(1000) NOT NULL DEFAULT '' COMMENT '文件路径' AFTER `param_md5`;
...
CREATE TABLE `pay_transfer` (
   ...
   `out_trade_no` varchar(50) NOT NULL DEFAULT '' COMMENT '外部账号单',
   `real_name` varchar(50) NOT NULL DEFAULT '' COMMENT '收款人姓名',
   `amount` decimal(10,2) NOT NULL DEFAULT '0.00' COMMENT '转账金额',
   `desc` varchar(255) NOT NULL DEFAULT '' COMMENT '转账说明',
   `transfer_type` varchar(50) NOT NULL DEFAULT '' COMMENT '收款账户类型 微信wechatpay 支付宝alipay 银行卡bank',
   `account_number` varchar(50) NOT NULL DEFAULT '' COMMENT '收款账户号',
   `is_weapp` int NOT NULL DEFAULT '0' COMMENT '是否小程序',
   `member_id` int NOT NULL DEFAULT '0' COMMENT '会员id',
   `from_type` varchar(50) NOT NULL DEFAULT '' COMMENT '来源类型',
   `status` int NOT NULL DEFAULT '0' COMMENT '转账状态 0：待转账 1：转账中 2：转账成功 3：转账失败 -1：已取消',
   `relate_tag` varchar(50) NOT NULL DEFAULT '' COMMENT '关联关键字',
   `resp_data` text COMMENT '返回数据',
...
CREATE TABLE `pay_offline` (
   `id` int NOT NULL AUTO_INCREMENT,
   `member_id` int NOT NULL DEFAULT '0' COMMENT '会员id',
   `out_trade_no` varchar(255) COLLATE utf8mb4_general_ci NOT NULL DEFAULT '' COMMENT '外部交易号',
   `desc` varchar(500) COLLATE utf8mb4_general_ci NOT NULL DEFAULT '' COMMENT '支付说明',
   `imgs` varchar(3000) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL DEFAULT '' COMMENT '支付图片，多张用逗号分割',
   `status` tinyint NOT NULL COMMENT '状态 0待审核 1审核通过 2审核拒绝 3已取消',
...
CREATE TABLE pay_balance (
  id int(11) NOT NULL AUTO_INCREMENT,
  auth_code varchar(18) NOT NULL DEFAULT '',
  site_id int(11) NOT NULL DEFAULT 0,
  member_id int(11) NOT NULL DEFAULT 0,
  create_time int(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
  expire_time int(11) NOT NULL DEFAULT 0 COMMENT '过期时间',
  dynamic_code varchar(4) NOT NULL DEFAULT '' COMMENT '动态码',
  PRIMARY KEY (id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
...
ALTER TABLE pay_balance ADD UNIQUE INDEX UK_ns_authcode_pay_auth_code (auth_code);
...
ALTER TABLE `pay` ADD COLUMN `is_delete` tinyint(1) NOT NULL DEFAULT 0 COMMENT '是否删除 1是 0否' AFTER `pay_json`;

ALTER TABLE `pay` ADD COLUMN `relate_id` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT '' COMMENT '关联id' AFTER `is_delete`;

ALTER TABLE `pay` MODIFY COLUMN `pay_status` int NOT NULL DEFAULT 0 COMMENT '支付状态（0.待支付 1. 支付中 2. 已支付 -1已取消 -2已关闭）' AFTER `pay_voucher`;
...
CREATE TABLE order_goods (
  ...
  sku_id int(11) NOT NULL DEFAULT 0 COMMENT '商品skuid',
  sku_name varchar(255) NOT NULL DEFAULT '' COMMENT '商品名称',
  sku_image varchar(2000) NOT NULL DEFAULT '' COMMENT '商品图片',
  sku_no varchar(255) NOT NULL DEFAULT '' COMMENT '商品编码',
  is_virtual int(11) NOT NULL DEFAULT 0 COMMENT '是否是虚拟商品',
  goods_class varchar(50) NOT NULL DEFAULT '0' COMMENT '商品种类(1.实物 2.虚拟3.卡券)',
  goods_class_name varchar(50) NOT NULL DEFAULT '' COMMENT '商品类型名称',
  ...
  store_id int(11) NOT NULL DEFAULT 0 COMMENT '所属门店',
  card_item_id int(11) NOT NULL DEFAULT 0 COMMENT '卡项id',
...
ALTER TABLE order_goods
  ADD COLUMN shop_active_refund_no VARCHAR(50) NOT NULL DEFAULT '' COMMENT '商家主动退款编号';

ALTER TABLE order_goods
  ADD COLUMN shop_active_refund_remark VARCHAR(255) NOT NULL DEFAULT '' COMMENT '商家主动退款说明';

ALTER TABLE order_goods
  ADD COLUMN shop_active_refund_money DECIMAL(10, 2) NOT NULL DEFAULT 0.00 COMMENT '商家主动退款金额';

ALTER TABLE order_goods
  ADD COLUMN shop_active_refund_money_type INT(11) NOT NULL DEFAULT 1 COMMENT '商家主动退款方式 1 原路退款 2线下退款3退款到余额';

ALTER TABLE `order_goods` ADD COLUMN `refund_images` varchar(3000) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT '' COMMENT '退款图片 最多五张' AFTER `refund_remark`;

ALTER TABLE order_goods
 MODIFY shop_active_refund TINYINT(4) NOT NULL DEFAULT 0 COMMENT '商家主动退款（0否  1是）' AFTER shop_refund_remark;
ALTER TABLE order_goods
 MODIFY shop_active_refund_no VARCHAR(50) NOT NULL DEFAULT '' COMMENT '商家主动退款编号' AFTER shop_active_refund;
ALTER TABLE order_goods
 MODIFY shop_active_refund_remark VARCHAR(255) NOT NULL DEFAULT '' COMMENT '商家主动退款说明' AFTER shop_active_refund_no;
ALTER TABLE order_goods
 MODIFY shop_active_refund_money DECIMAL(10, 2) NOT NULL DEFAULT 0.00 COMMENT '商家主动退款金额' AFTER shop_active_refund_remark;
ALTER TABLE order_goods
 MODIFY shop_active_refund_money_type INT(11) NOT NULL DEFAULT 1 COMMENT '商家主动退款方式 1 原路退款 2线下退款3退款到余额' AFTER shop_active_refund_money;
...
CREATE TABLE `order` (
  ...
  goods_num DECIMAL(12, 3) NOT NULL DEFAULT 0.000 COMMENT '商品件数',
  delivery_store_id int(11) NOT NULL DEFAULT 0 COMMENT '门店id',
  ...
  cashier_order_type varchar(50) NOT NULL DEFAULT '' COMMENT '收银台订单类型  goods  商品相关  card 卡项   recharge 充值',
  cashier_sell_time int(11) NOT NULL DEFAULT 0 COMMENT '销售时间',
  cashier_operator_id int(11) UNSIGNED NOT NULL DEFAULT 0 COMMENT '收银台操作人',
  cashier_operator_name varchar(255) NOT NULL DEFAULT '' COMMENT '收银台操作人',
  balance decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '为收银台新创建的字段,切勿随意使用',
  total_balance decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '为收银台新创建的字段,切勿随意使用',
  store_id int(11) NOT NULL DEFAULT 0 COMMENT '所属门店',
  reduction decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '收银订单减免金额',
  round_money decimal(10, 0) NOT NULL DEFAULT 0 COMMENT '抹零金额',
  order_scene varchar(50) NOT NULL DEFAULT 'online' COMMENT '订单场景   online  线上  cashier 收银台',
  store_commission_rate decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '门店比率1',
...
CREATE TABLE notes (
  ...
  sort int(11) NOT NULL DEFAULT 0 COMMENT '排序',
  release_time int(11) NOT NULL DEFAULT 0 COMMENT '发布时间',
  note_link varchar(255) NOT NULL DEFAULT '' COMMENT '公众号文章链接',
  video_path varchar(255) NOT NULL DEFAULT '' COMMENT '视频地址',
  PRIMARY KEY (note_id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
...
CREATE TABLE message (
  ...
  weapp_template_id varchar(1000) NOT NULL DEFAULT '' COMMENT '微信小程序配置参数',
  aliapp_is_open int NOT NULL DEFAULT 0 COMMENT '支付宝小程序是否启动',
  aliapp_template_id varchar(1000) NOT NULL DEFAULT '' COMMENT '支付宝小程序配置参数',
  PRIMARY KEY (id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
...
ALTER TABLE menu ADD INDEX IDX_ns_menu_level (level);
...
ALTER TABLE `menu` ADD COLUMN `type` varchar(20) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT 'page' COMMENT '类型 页面 page 按钮 button' AFTER `is_control`;
...
CREATE TABLE member_recommend_award (
  award_id int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
  site_id int(11) NOT NULL DEFAULT 0 COMMENT '站点ID',
  recommend_id int(11) NOT NULL DEFAULT 0 COMMENT '奖励活动ID',
  recommend_name varchar(255) NOT NULL DEFAULT '' COMMENT '奖励活动名称',
  member_id int(11) NOT NULL DEFAULT 0 COMMENT '奖励人ID',
  member_nickname varchar(255) NOT NULL DEFAULT '' COMMENT '奖励人昵称',
  source_member int(11) NOT NULL DEFAULT 0 COMMENT '被邀请人ID',
  source_member_nickname varchar(255) NOT NULL DEFAULT '' COMMENT '被邀请人昵称',
  create_time int(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
  remark varchar(255) NOT NULL DEFAULT '' COMMENT '备注',
  point decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '赠送积分',
  balance decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '赠送的余额',
  coupon varchar(255) NOT NULL DEFAULT '' COMMENT '优惠券ID',
  coupon_num int(11) NOT NULL DEFAULT 0 COMMENT '已发放优惠券数量',
  PRIMARY KEY (award_id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
...
CREATE TABLE member_recommend (
  ...
  site_id int(11) NOT NULL DEFAULT 0 COMMENT '站点ID',
  recommend_name varchar(255) NOT NULL DEFAULT '' COMMENT '活动名称',
  start_time int(11) NOT NULL DEFAULT 0 COMMENT '邀请奖励 开始时间',
  end_time int(11) NOT NULL DEFAULT 0 COMMENT '邀请奖励 结束时间',
  create_time int(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
  update_time int(11) NOT NULL DEFAULT 0 COMMENT '修改时间',
  status int(11) NOT NULL DEFAULT 0 COMMENT '状态（0未开始1进行中2已结束-1已关闭）',
  point int(11) NOT NULL DEFAULT 0 COMMENT '积分',
  ...
  max_coupon int(11) NOT NULL DEFAULT 0 COMMENT '每月最多优惠券数量',
  remark varchar(255) NOT NULL DEFAULT '' COMMENT '备注',
  type varchar(255) NOT NULL DEFAULT '' COMMENT '奖励方式',
  max_fetch int(11) UNSIGNED DEFAULT 0 COMMENT '每人邀请奖励上限 0为不限制',
...
CREATE TABLE member_recharge_order (
  ...
  order_from_name varchar(255) NOT NULL DEFAULT '' COMMENT '订单来源名称',
  store_id int(11) NOT NULL DEFAULT 0 COMMENT '门店id',
  relate_id int(11) NOT NULL DEFAULT 0 COMMENT '业务id',
  relate_type varchar(255) NOT NULL DEFAULT '' COMMENT '业务类型',
  PRIMARY KEY (order_id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
...
CREATE TABLE member_level_records (
  ...
  change_type varchar(255) NOT NULL DEFAULT '' COMMENT '变更方式',
  change_type_name varchar(255) NOT NULL DEFAULT '' COMMENT '变更方式名称',
  PRIMARY KEY (id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
...
CREATE TABLE member_level_order (
  ...
  nickname varchar(255) NOT NULL DEFAULT '' COMMENT '购买人昵称',
...
CREATE TABLE member_level (
  ...
  is_recommend int(11) NOT NULL DEFAULT 0 COMMENT '是否推荐',
  level_background varchar(100) NOT NULL DEFAULT '' COMMENT '背景色',
  level_text_color varchar(100) NOT NULL DEFAULT '' COMMENT '文字颜色',
  level_picture varchar(255) NOT NULL DEFAULT '' COMMENT '背景图',
...
CREATE TABLE member_goods_card_records (
  id int(11) NOT NULL AUTO_INCREMENT,
  card_id int(11) NOT NULL DEFAULT 0,
  site_id int(11) NOT NULL DEFAULT 0,
  card_item_id int(11) NOT NULL DEFAULT 0,
  type varchar(255) NOT NULL DEFAULT '' COMMENT 'verify核销order订单',
  relation_id int(11) NOT NULL DEFAULT 0 COMMENT '关联id 核销码id orderid',
  create_time int(11) NOT NULL DEFAULT 0,
  store_id int(11) NOT NULL DEFAULT 0 COMMENT '使用门店',
  num int(11) NOT NULL DEFAULT 1 COMMENT '使用次数',
  PRIMARY KEY (id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
...
CREATE TABLE member_goods_card_item (
  item_id int(11) NOT NULL AUTO_INCREMENT,
  site_id int(11) NOT NULL DEFAULT 0 COMMENT '站点id',
  card_id int(11) NOT NULL DEFAULT 0 COMMENT '会员卡项id',
  member_id int(11) NOT NULL DEFAULT 0 COMMENT '会员id',
  goods_id int(11) NOT NULL DEFAULT 0 COMMENT '服务商品id',
  sku_id int(11) NOT NULL DEFAULT 0 COMMENT '服务商品规格id',
  goods_class varchar(255) NOT NULL DEFAULT '' COMMENT '商品类型',
  num int(11) NOT NULL DEFAULT 0 COMMENT '商品次数/数量',
  use_num int(11) NOT NULL DEFAULT 0 COMMENT '使用次数/数量',
  end_time int(11) NOT NULL DEFAULT 0 COMMENT '有效期',
  member_verify_id int(11) NOT NULL DEFAULT 0 COMMENT '会员核销码id',
  store_id int(11) NOT NULL DEFAULT 0 COMMENT '门店id',
  card_type varchar(255) NOT NULL DEFAULT '' COMMENT '卡项类型',
  PRIMARY KEY (item_id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
...
CREATE TABLE member_goods_card (
  card_id int(11) NOT NULL AUTO_INCREMENT,
  site_id int(11) NOT NULL DEFAULT 0 COMMENT '站点id',
  member_id int(11) NOT NULL DEFAULT 0 COMMENT '会员id',
  goods_id int(11) NOT NULL DEFAULT 0 COMMENT '卡项商品id',
  goods_name varchar(2000) NOT NULL DEFAULT '' COMMENT '卡项名称',
  create_time int(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
  end_time int(11) NOT NULL DEFAULT 0 COMMENT '到期时间',
  card_code varchar(50) NOT NULL DEFAULT '' COMMENT '卡号',
  order_id int(11) NOT NULL DEFAULT 0 COMMENT '关联订单id',
  total_num int(11) NOT NULL DEFAULT 0 COMMENT '卡项总次数',
  total_use_num int(11) NOT NULL DEFAULT 0 COMMENT '卡项使用次数',
  card_type varchar(255) NOT NULL DEFAULT '' COMMENT '卡项类型',
...
ALTER TABLE `member_goods_card` ADD COLUMN `invalid_reason` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT '' COMMENT '失效原因' AFTER `store_id`;
...
CREATE TABLE member_cluster (
  ...
  rule_json text DEFAULT NULL COMMENT '规则json',
  member_num int(11) NOT NULL DEFAULT 0 COMMENT '群体内会员 数量',
  member_ids text DEFAULT NULL COMMENT '群体内会员ID',
  create_time int(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
  update_time int(11) NOT NULL DEFAULT 0 COMMENT '更新时间',
...
CREATE TABLE member_cancel (
  ...
  member_json text DEFAULT NULL COMMENT '会员信息',
  fenxiao_json text DEFAULT NULL COMMENT '分销信息',
  PRIMARY KEY (id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
...
CREATE TABLE member_account (
  ...
  username varchar(255) NOT NULL DEFAULT '' COMMENT '用户名',
  mobile varchar(255) NOT NULL DEFAULT '' COMMENT '手机',
  order_number varchar(255) NOT NULL DEFAULT '' COMMENT '订单消费和退款的订单号',
  email varchar(255) NOT NULL DEFAULT '' COMMENT '邮箱',
  related_id int(11) NOT NULL DEFAULT 0 COMMENT '关联Id（拼团返利使用）',
  point_expire tinyint(4) NOT NULL DEFAULT 0 COMMENT '积分到期状态',
  PRIMARY KEY (id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
...
CREATE TABLE member (
  ...
  longitude varchar(255) NOT NULL DEFAULT '' COMMENT '经度',
  latitude varchar(255) NOT NULL DEFAULT '' COMMENT '纬度',
  member_code varchar(50) NOT NULL DEFAULT '' COMMENT '会员卡号',
  bind_fenxiao_time int(11) NOT NULL DEFAULT 0 COMMENT '会员与分销商绑定关系的时间',
...
ALTER TABLE member ADD INDEX IDX_ns_member_is_delete (is_delete);

ALTER TABLE member ADD INDEX IDX_ns_member_site_id (site_id);

ALTER TABLE member ADD INDEX IDX_ns_member_status (status);
...
ALTER TABLE member ADD INDEX IDX_member_balance(balance);

ALTER TABLE member ADD INDEX IDX_member_balance_money(balance_money);

ALTER TABLE member ADD INDEX IDX_member_code(member_code);

ALTER TABLE member ADD INDEX IDX_member_is_delete(is_delete);

ALTER TABLE member ADD INDEX IDX_member_last_login_time(last_login_time);

ALTER TABLE member ADD INDEX IDX_member_member_label(member_label);

ALTER TABLE member ADD INDEX IDX_member_member_level(member_level);

ALTER TABLE member ADD INDEX IDX_member_order_money(order_money);

ALTER TABLE member ADD INDEX IDX_member_point(point);

ALTER TABLE member ADD INDEX IDX_member_reg_time(reg_time);
...
ALTER TABLE `local_delivery_package` MODIFY COLUMN `goods_id_array` text CHARACTER SET utf8 COLLATE utf8_general_ci NULL COMMENT '商品组合列表' AFTER `order_goods_id_array`;
...
CREATE TABLE `local` (
  ...
  continued_distance decimal(10, 2) NOT NULL DEFAULT 1.00 COMMENT '每增加多少距离',
...
ALTER TABLE `local`
  CHANGE COLUMN local_area_json local_area_json TEXT DEFAULT NULL COMMENT '区域配送设置';
...
CREATE TABLE link (
  ...
  support_diy_view varchar(255) NOT NULL DEFAULT '' COMMENT '支持的自定义页面（为空表示都支持）',
  PRIMARY KEY (id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
...
CREATE TABLE goods_virtual (
  ...
  expire_time int(11) NOT NULL DEFAULT 0 COMMENT '核销有效期 0永久 ',
  card_info varchar(2500) NOT NULL DEFAULT '' COMMENT '卡密信息',
  sold_time int(11) NOT NULL DEFAULT 0 COMMENT '售出时间',
  goods_id int(11) NOT NULL DEFAULT 0 COMMENT '商品id',
  verify_total_count int(11) NOT NULL DEFAULT 0 COMMENT '核销次数',
  verify_use_num int(11) NOT NULL DEFAULT 0 COMMENT '已核销次数',
  PRIMARY KEY (id)
)
ENGINE = INNODB,
...
CREATE TABLE goods_sku (
  ...
  start_time int(11) NOT NULL DEFAULT 0 COMMENT '活动开始时间',
  end_time int(11) NOT NULL DEFAULT 0 COMMENT '活动结束时间',
  stock DECIMAL(12, 3) NOT NULL DEFAULT 0.000 COMMENT '商品sku库存',
  ...
  sale_num DECIMAL(12, 3) NOT NULL DEFAULT 0.000 COMMENT '销量',
  collect_num int(11) NOT NULL DEFAULT 0 COMMENT '收藏量',
  sku_image varchar(255) NOT NULL DEFAULT '' COMMENT 'sku主图',
  sku_images varchar(2000) NOT NULL DEFAULT '' COMMENT 'sku图片',
  goods_class int(11) NOT NULL DEFAULT 1 COMMENT '商品种类1.实物商品2.虚拟商品3.卡券商品4.服务项目5.卡项商品',
  ...
  virtual_sale DECIMAL(12, 3) NOT NULL DEFAULT 0.000 COMMENT '虚拟销量',
  ...
  sale_channel varchar(50) NOT NULL DEFAULT 'all' COMMENT '销售渠道 all 线上线下销售 online 线上销售  offline线下销售',
  sale_store varchar(5000) NOT NULL DEFAULT 'all' COMMENT '适用门店 all 全部门店 ,门店id,门店id, 部分门店',
  service_length int(11) DEFAULT 0 COMMENT '服务时长分钟',
  real_stock DECIMAL(12, 3) NOT NULL DEFAULT 0.000 COMMENT '实物库存',
  is_unify_price int(11) NOT NULL DEFAULT 1 COMMENT '是否统一销售价',
  plu varchar(4) NOT NULL DEFAULT '' COMMENT 'plu码',
  pricing_type varchar(10) NOT NULL DEFAULT 'num' COMMENT '计价方式 num 计件 weight 计重',
  PRIMARY KEY (sku_id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
...
ALTER TABLE `goods_sku` MODIFY COLUMN `weight` decimal(12, 3) NOT NULL DEFAULT 0.000 COMMENT '重量（单位g）' AFTER `stock`;

ALTER TABLE `goods_sku` MODIFY COLUMN `volume` decimal(12, 3) NOT NULL DEFAULT 0.000 COMMENT '体积（单位立方米）' AFTER `weight`;
...
CREATE TABLE goods_import_record (
  id int(11) NOT NULL AUTO_INCREMENT,
  site_id int(11) NOT NULL DEFAULT 0 COMMENT '站点ID',
  record_name varchar(255) NOT NULL DEFAULT '' COMMENT '记录名称',
  import_time int(11) NOT NULL DEFAULT 0 COMMENT '导入时间',
  success_num int(11) NOT NULL DEFAULT 0 COMMENT '成功数',
  fail_num int(11) NOT NULL DEFAULT 0 COMMENT '失败数',
  fail_data text DEFAULT NULL COMMENT '失败数据',
  data text DEFAULT NULL COMMENT '导入原始数据',
  PRIMARY KEY (id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
...
ALTER TABLE `goods_grab_detail` MODIFY COLUMN `url` varchar(3000) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT '' COMMENT '采集地址' AFTER `grab_id`;
...
CREATE TABLE goods_evaluate (
  ...
  is_audit int(11) NOT NULL DEFAULT 1 COMMENT '审核状态：0 未审核 1 审核通过，2 审核拒绝',
  again_is_audit int(11) NOT NULL DEFAULT 0 COMMENT '审核状态：0 未审核 1 审核通过，2 审核拒绝',
...
ALTER TABLE `goods_evaluate`
  CHANGE COLUMN content content VARCHAR(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL DEFAULT '' COMMENT '评价内容';

ALTER TABLE `goods_evaluate`
  CHANGE COLUMN again_content again_content VARCHAR(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL DEFAULT '' COMMENT '追加评价内容';
...
CREATE TABLE goods_community_qrcode (
  ...
  qr_state tinyint(4) NOT NULL DEFAULT 0 COMMENT '二维码状态(0未启用1启用)',
  create_time int(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
  modify_time int(11) NOT NULL DEFAULT 0 COMMENT '修改时间',
  site_id int(11) NOT NULL DEFAULT 0 COMMENT '所属店铺id',
  PRIMARY KEY (qr_id)
...
CREATE TABLE goods_category (
  ...
  link_url varchar(2000) NOT NULL DEFAULT '' COMMENT '广告链接',
  is_recommend int(11) NOT NULL DEFAULT 0 COMMENT '是否推荐',
  icon varchar(255) NOT NULL DEFAULT '' COMMENT '图标',
  PRIMARY KEY (category_id)
)
...
CREATE TABLE goods_card_item (
  id int(11) NOT NULL AUTO_INCREMENT,
  site_id int(11) NOT NULL DEFAULT 0,
  card_goods_id int(11) NOT NULL DEFAULT 0,
  goods_id int(11) NOT NULL DEFAULT 0,
  sku_id int(11) NOT NULL DEFAULT 0,
  num int(11) NOT NULL DEFAULT 0 COMMENT '次卡可用次数',
  discount decimal(10, 2) NOT NULL DEFAULT 100.00 COMMENT '折扣卡折扣',
  PRIMARY KEY (id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
...
ALTER TABLE goods_card_item ADD INDEX IDX_ns_goods_card_item_card_goods_id (card_goods_id);

ALTER TABLE goods_card_item ADD INDEX IDX_ns_goods_card_item_sku_id (sku_id);
...
CREATE TABLE goods_card (
  ...
  card_type_name varchar(255) NOT NULL DEFAULT '' COMMENT '卡类型名称',
  renew_price decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '续费价格',
  recharge_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '充值到账金额',
  common_num int(11) NOT NULL DEFAULT 0 COMMENT '通卡总可用次数',
  discount_goods_type varchar(50) NOT NULL DEFAULT 'all' COMMENT '折扣卡适用商品类型 all 全部商品 part 部分商品',
  discount decimal(10, 2) NOT NULL DEFAULT 100.00 COMMENT '折扣卡全部商品时的折扣',
  validity_type smallint(6) NOT NULL DEFAULT 0 COMMENT '有效期类型 0永久有效 1购买后几日有效 2指定有效时间',
  validity_day int(11) NOT NULL DEFAULT 0 COMMENT '有效期天数',
...
ALTER TABLE goods_card ADD INDEX IDX_ns_goods_card_card_type (card_type);

ALTER TABLE goods_card ADD UNIQUE INDEX UK_ns_goods_card_goods_id (goods_id);
...
CREATE TABLE goods_brand (
  ...
  image_url VARCHAR(255) NOT NULL DEFAULT '' COMMENT '品牌logo',
  banner VARCHAR(255) NOT NULL DEFAULT '' COMMENT '品牌广告图',
  brand_desc VARCHAR(1000) NOT NULL DEFAULT '' COMMENT '品牌简介',
  site_id int(11) NOT NULL DEFAULT 0 COMMENT '所属店铺id',
  sort int(11) NOT NULL DEFAULT 0 COMMENT '排序',
  create_time int(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
...
CREATE TABLE goods_attribute (
  ...
  attr_value_format TEXT DEFAULT NULL COMMENT '属性值格式json',
...
CREATE TABLE goods (
  ...
  goods_image varchar(2000) NOT NULL DEFAULT '' COMMENT '商品主图路径',
  goods_content text DEFAULT NULL COMMENT '商品详情',
  goods_state tinyint(4) NOT NULL DEFAULT 1 COMMENT '商品状态（1.正常0下架）',
  ...
  goods_stock DECIMAL(12, 3) NOT NULL DEFAULT 0.000 COMMENT '商品库存（总和）',
  goods_stock_alarm int(11) NOT NULL DEFAULT 0 COMMENT '库存预警',
  is_virtual tinyint(4) NOT NULL DEFAULT 0 COMMENT '是否虚拟类商品（0实物1.虚拟）',
  virtual_indate int(11) NOT NULL DEFAULT 1 COMMENT '虚拟商品有效期',
  is_free_shipping tinyint(4) NOT NULL DEFAULT 0 COMMENT '是否免邮',
  ...
  sale_channel varchar(50) NOT NULL DEFAULT 'all' COMMENT '销售渠道 all 线上线下销售 online 线上销售  offline线下销售',
  sale_store varchar(5000) NOT NULL DEFAULT 'all' COMMENT '适用门店 all 全部门店 ,门店id,门店id, 部分门店',
  service_category varchar(2000) NOT NULL DEFAULT '' COMMENT '服务类目',
  is_unify_price int(11) NOT NULL DEFAULT 1 COMMENT '是否统一销售价',
  real_stock DECIMAL(12, 3) NOT NULL DEFAULT 0.000 COMMENT '实物库存',
  pricing_type varchar(10) NOT NULL DEFAULT 'num' COMMENT '计价方式 num 计件 weight 计重',
...
ALTER TABLE goods
  ADD INDEX IDX_ns_goods_brand(brand_id);

ALTER TABLE goods
  ADD INDEX IDX_ns_goods_name(goods_name);

ALTER TABLE goods
  ADD INDEX IDX_ns_goods_price(price);

ALTER TABLE goods
  ADD INDEX IDX_ns_goods_real_stock(real_stock);

ALTER TABLE goods
  ADD INDEX IDX_ns_goods_sale_channel(sale_channel);

ALTER TABLE goods
  ADD INDEX IDX_ns_goods_sale_store(sale_store(1024));

ALTER TABLE goods
  ADD INDEX IDX_ns_goods_si_virtual(is_virtual);

ALTER TABLE goods
  ADD INDEX IDX_ns_goods_state(goods_state);
...
CREATE TABLE giftcard (
  ...
  is_delete int(11) NOT NULL DEFAULT 0 COMMENT '0未删除1已删除',
  is_allow_transfer int(11) NOT NULL DEFAULT 0 COMMENT '是否允许转赠',
  status INT(11) NOT NULL DEFAULT 1 COMMENT '0：已结束，1：进行中',
  create_time int(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
  update_time int(11) NOT NULL DEFAULT 0 COMMENT '修改时间',
  ...
  instruction text DEFAULT NULL COMMENT '说明',
...
CREATE TABLE form_data (
  ...
  site_id int(11) NOT NULL DEFAULT 0 COMMENT '站点id',
  form_id int(11) NOT NULL DEFAULT 0 COMMENT '表单id',
  member_id int(11) NOT NULL DEFAULT 0 COMMENT '所属会员id',
  relation_id int(11) NOT NULL DEFAULT 0 COMMENT '表单关联id',
  create_time int(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
  form_data text DEFAULT NULL COMMENT '表单数据',
  scene varchar(255) NOT NULL DEFAULT '' COMMENT '场景 order订单',
  PRIMARY KEY (id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
...
ALTER TABLE form_data ADD INDEX UK_ns_form_data (relation_id, scene);
...
CREATE TABLE fenxiao_withdraw (
  ...
  transfer_type varchar(30) NOT NULL DEFAULT '1' COMMENT '转账方式',
  ...
  document varchar(1000) NOT NULL DEFAULT '' COMMENT '转账单据',
  audit_time int(11) NOT NULL DEFAULT 0 COMMENT '审核时间',
  refuse_reason varchar(255) NOT NULL DEFAULT '' COMMENT '拒绝理由',
  applet_type int(11) NOT NULL DEFAULT 0,
  PRIMARY KEY (id)
...
ALTER TABLE `fenxiao_withdraw` ADD COLUMN `fail_reason` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT '' COMMENT '失败原因' AFTER `applet_type`;
...
CREATE TABLE express_template_free_shipping (
  ...
  area_ids mediumtext DEFAULT NULL COMMENT '地址id序列',
  area_names mediumtext DEFAULT NULL COMMENT '地址名称序列',
  snum int(11) NOT NULL DEFAULT 0 COMMENT '件数',
  sprice decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '价格',
  PRIMARY KEY (item_id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
...
CREATE TABLE express_template (
  ...
  appoint_free_shipping int(11) NOT NULL DEFAULT 0 COMMENT '是否指定免邮',
  shipping_surplus_area_ids mediumtext DEFAULT NULL COMMENT '包邮地区剩余地址id',
  PRIMARY KEY (template_id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
...
CREATE TABLE express_delivery_package (
  ...
  order_id int(11) NOT NULL DEFAULT 0 COMMENT '订单id',
  order_goods_id_array varchar(1000) NOT NULL DEFAULT '' COMMENT '订单项商品组合列表',
  goods_id_array text DEFAULT NULL COMMENT '商品组合列表',
  package_name varchar(50) NOT NULL DEFAULT '' COMMENT '包裹名称（包裹- 1 包裹 - 2）',
...
ALTER TABLE `express_delivery_package` ADD COLUMN `trace` text CHARACTER SET utf8 COLLATE utf8_general_ci NULL COMMENT '物流轨迹' AFTER `template_name`;
...
CREATE TABLE `export` (
 ...
 `from_type` varchar(50) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT '' COMMENT '来源方式',
 `from_type_name` varchar(50) NOT NULL DEFAULT '' COMMENT '来源方式名称',
 `condition` varchar(2000) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT '' COMMENT '条件  json',
 `status` int NOT NULL DEFAULT '0' COMMENT '状态  0 导出中 1 导出成功  2 导出失败',
 `fail_log` varchar(2000) CHARACTER SET utf8 COLLATE utf8_general_ci DEFAULT '' COMMENT '失败日志',
 `create_time` int NOT NULL DEFAULT '0' COMMENT '导出时间',
 `path` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT '' COMMENT '导出文件的物理路径',
 `site_id` int NOT NULL DEFAULT '0' COMMENT '站点id',
 `store_id` int NOT NULL DEFAULT '0' COMMENT '门店id',
 `data_num` int NOT NULL DEFAULT '0' COMMENT '导出数据条数',
...
CREATE TABLE diy_theme (
  ...
  addon_name varchar(255) NOT NULL DEFAULT '' COMMENT '插件标识',
  main_color varchar(50) NOT NULL DEFAULT '' COMMENT '主色调',
  aux_color varchar(50) NOT NULL DEFAULT '' COMMENT '辅色调',
  preview varchar(550) NOT NULL DEFAULT '' COMMENT '预览图，多个逗号隔开',
  color_img varchar(255) NOT NULL DEFAULT '' COMMENT '配色图片',
  value text DEFAULT NULL COMMENT '其他配色',
  PRIMARY KEY (id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
COLLATE utf8_general_ci,
COMMENT = '自定义模板主题风格配色表';

ALTER TABLE diy_theme ADD INDEX addon_name (addon_name);

CREATE TABLE diy_template_goods_item (
  goods_item_id int(11) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键',
  goods_id int(11) NOT NULL DEFAULT 0 COMMENT '模板组id',
  title varchar(255) NOT NULL DEFAULT '' COMMENT '名称',
  name varchar(255) NOT NULL DEFAULT '' COMMENT '所属页面（首页、分类，空为微页面）',
  addon_name varchar(255) NOT NULL DEFAULT '' COMMENT '插件标识',
  value longtext DEFAULT NULL COMMENT '模板数据',
  create_time int(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
  modify_time int(11) NOT NULL DEFAULT 0 COMMENT '修改时间',
...
ALTER TABLE diy_template_goods_item ADD INDEX addon_name (addon_name);

CREATE TABLE diy_template_goods (
  goods_id int(11) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键',
  goods_item_id int(11) NOT NULL DEFAULT 0 COMMENT '装修的页面项id，默认取第一个页面',
  title varchar(255) NOT NULL DEFAULT '' COMMENT '模板名称',
  name varchar(255) NOT NULL DEFAULT '' COMMENT '模板标识',
  addon_name varchar(255) NOT NULL DEFAULT '' COMMENT '插件标识',
  cover varchar(255) NOT NULL DEFAULT '' COMMENT '封面图',
  preview varchar(255) NOT NULL DEFAULT '' COMMENT '预览图',
  `desc` varchar(255) NOT NULL DEFAULT '' COMMENT '模版描述',
  category_id int(11) NOT NULL DEFAULT 0 COMMENT '模板分类id',
  category_name varchar(255) NOT NULL DEFAULT '' COMMENT '模板分类名称',
...
ALTER TABLE diy_template_goods ADD INDEX addon_name (addon_name);

ALTER TABLE diy_template_goods ADD UNIQUE INDEX name (name);

CREATE TABLE diy_template_category (
  category_id int(11) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键',
  name varchar(255) NOT NULL DEFAULT '' COMMENT '名称',
  pid int(11) NOT NULL DEFAULT 0 COMMENT '上级分类id',
  level int(11) NOT NULL DEFAULT 0 COMMENT '层级',
  state int(11) NOT NULL DEFAULT 1 COMMENT '状态（是否展示）',
  create_time int(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
  modify_time int(11) NOT NULL DEFAULT 0 COMMENT '修改时间',
  sort int(11) NOT NULL DEFAULT 0 COMMENT '排序',
  PRIMARY KEY (category_id)
...
CREATE TABLE diy_template (
  ...
  addon_name varchar(255) NOT NULL DEFAULT '' COMMENT '插件标识',
  value longtext DEFAULT NULL COMMENT '默认值',
...
ALTER TABLE diy_template ADD INDEX addon_name (addon_name);

ALTER TABLE diy_template ADD UNIQUE INDEX name (name);
...
ALTER TABLE `cron` ADD COLUMN `error_num` int NOT NULL DEFAULT 0 COMMENT '错误次数' AFTER `create_time`;

ALTER TABLE `cron` ADD COLUMN `status` tinyint(1) NOT NULL DEFAULT 0 COMMENT '状态 0待执行 1执行中' AFTER `error_num`;

ALTER TABLE `cron` MODIFY COLUMN `relate_id` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT '' COMMENT '关联关键字id' AFTER `execute_time`;
...
CREATE TABLE change_shifts_record (
  id int(11) NOT NULL AUTO_INCREMENT,
  site_id int(11) NOT NULL DEFAULT 0 COMMENT '所属站点',
  store_id int(11) NOT NULL DEFAULT 0 COMMENT '所属门店',
  uid int(11) NOT NULL DEFAULT 0 COMMENT '交班员工',
  start_time int(11) NOT NULL DEFAULT 0 COMMENT '交班 班次开始时间',
  end_time int(11) NOT NULL DEFAULT 0 COMMENT '交班 班次结束时间',
  billing_count int(11) NOT NULL DEFAULT 0 COMMENT '开单数量',
  billing_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '开单金额',
  buycard_count int(11) NOT NULL DEFAULT 0 COMMENT '办卡数量',
  buycard_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '办卡金额',
  ...
  refund_count int(11) NOT NULL DEFAULT 0 COMMENT '退款数量',
  refund_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '退款金额',
  cash decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '现金收款金额',
  alipay decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '支付宝线上收款金额',
  wechatpay decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '微信线上收款金额',
  own_wechatpay decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '个人微信收款金额',
  own_alipay decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '个人支付宝收款金额',
  own_pos decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '个人pos刷卡收款金额',
  cash_count int(11) NOT NULL DEFAULT 0 COMMENT '现金收款数量',
  alipay_count int(11) NOT NULL DEFAULT 0 COMMENT '支付宝收款数量',
  wechatpay_count int(11) NOT NULL DEFAULT 0 COMMENT '微信收款数量',
  own_wechatpay_count int(11) NOT NULL DEFAULT 0 COMMENT '个人微信收款数量',
  own_alipay_count int(11) NOT NULL DEFAULT 0 COMMENT '各人支付宝收款数量',
  own_pos_count int(11) NOT NULL DEFAULT 0 COMMENT '个人pos收款数量',
  PRIMARY KEY (id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
...
CREATE TABLE cashier_pendorder_goods (
  order_goods_id int(11) NOT NULL AUTO_INCREMENT,
  order_id int(11) NOT NULL DEFAULT 0,
  sku_id int(11) NOT NULL DEFAULT 0,
  num DECIMAL(12, 3) NOT NULL DEFAULT 0.000,
  goods_id int(11) NOT NULL DEFAULT 0,
  price decimal(10, 2) NOT NULL DEFAULT 0.00,
  site_id int(11) NOT NULL DEFAULT 0,
  store_id INT(11) NOT NULL DEFAULT 0,
  goods_class varchar(255) NOT NULL DEFAULT '',
  PRIMARY KEY (order_goods_id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
...
CREATE TABLE cashier_pendorder (
  order_id int(11) NOT NULL AUTO_INCREMENT,
  site_id int(11) NOT NULL DEFAULT 0 COMMENT '站点id',
  store_id int(11) NOT NULL DEFAULT 0 COMMENT '门店id',
  member_id int(11) NOT NULL DEFAULT 0 COMMENT '会员id 0为散客',
  create_time int(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
  remark varchar(2500) NOT NULL DEFAULT '' COMMENT '备注',
  order_money decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '订单金额',
...
CREATE TABLE cashier_auth_group (
  group_id int(11) NOT NULL AUTO_INCREMENT,
  group_name varchar(255) NOT NULL DEFAULT '' COMMENT '权限组名称',
  menu_array text DEFAULT NULL COMMENT '权限集',
...
CREATE TABLE cashier_auth (
  id int(11) NOT NULL AUTO_INCREMENT,
  name varchar(200) NOT NULL DEFAULT '' COMMENT '权限关键字',
  title varchar(255) NOT NULL DEFAULT '' COMMENT '权限标题',
  type varchar(255) NOT NULL DEFAULT '' COMMENT '类型 page 页面 api 操作',
  parent varchar(255) NOT NULL DEFAULT '',
  url varchar(2500) NOT NULL DEFAULT '' COMMENT '权限内容',
  addon varchar(255) NOT NULL DEFAULT '' COMMENT '所属插件',
  PRIMARY KEY (id)
)
ENGINE = INNODB,
...
ALTER TABLE cashier_auth ADD UNIQUE INDEX UK_ns_cashier_auth_name (name);
...
CREATE TABLE blindbox_goods (
  ...
  sku_id int(11) NOT NULL DEFAULT 0 COMMENT '商品SKUID',
  site_id int(11) NOT NULL DEFAULT 0 COMMENT '站点Id',
  status int(11) NOT NULL DEFAULT 0 COMMENT '状态 0 未拆 1已拆',
  address_id int(11) NOT NULL DEFAULT 0 COMMENT '地址Id',
  sort int(11) NOT NULL DEFAULT 0 COMMENT '排序',
  member_id int(11) NOT NULL DEFAULT 0 COMMENT '用户Id',
  create_time int(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
...
CREATE TABLE blindbox_category (
  ...
  sort int(11) NOT NULL DEFAULT 0 COMMENT '排序',
  status int(11) NOT NULL DEFAULT 1 COMMENT '分类状态 1显示 2隐藏',
  site_id int(11) NOT NULL DEFAULT 0 COMMENT '站点Id',
  create_time int(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
  PRIMARY KEY (category_id)
)
...
CREATE TABLE blindbox (
  ...
  blindbox_name varchar(128) NOT NULL DEFAULT '' COMMENT '盲盒名称',
  goods_ids varchar(128) NOT NULL DEFAULT '' COMMENT '商品组',
  blindbox_images varchar(255) NOT NULL DEFAULT '' COMMENT '盲盒封面',
  blindbox_count int(11) NOT NULL DEFAULT 0 COMMENT '盲盒放置总量',
  blindbox_inventory int(11) NOT NULL DEFAULT 0 COMMENT '盲盒库存',
  blindbox_num int(11) NOT NULL DEFAULT 0 COMMENT '已拆数量',
  price decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '拆盒价格',
  new_price decimal(10, 2) NOT NULL DEFAULT 0.00 COMMENT '新人拆盒价',
  blindbox_status int(11) NOT NULL DEFAULT 0 COMMENT '状态 0未开始 1 进行中 2已结束 -1已关闭',
  start_time int(11) NOT NULL DEFAULT 0 COMMENT '开始时间',
  end_time int(11) NOT NULL DEFAULT 0 COMMENT '结束时间',
  remark varchar(255) NOT NULL DEFAULT '' COMMENT '说明',
  sort int(11) NOT NULL DEFAULT 0 COMMENT '排序',
  is_emptybox int(11) NOT NULL DEFAULT 1 COMMENT '空盒是否显示1显示 2不显示',
  is_balance int(11) NOT NULL DEFAULT 0 COMMENT '是否用余额支付',
  site_id int(11) NOT NULL DEFAULT 0 COMMENT '站点Id',
  early_inventory int(11) NOT NULL DEFAULT 0 COMMENT '预警库存',
  create_time int(11) NOT NULL DEFAULT 0 COMMENT '创建时间',
  PRIMARY KEY (blindbox_id)
)
ENGINE = INNODB,
CHARACTER SET utf8,
COLLATE utf8_general_ci,
...
CREATE TABLE article_category (
  ...
  sort int(11) NOT NULL DEFAULT 0 COMMENT '排序',
  article_num int(11) NOT NULL DEFAULT 0 COMMENT '文章数',
  create_time int(11) NOT NULL DEFAULT 0,
  update_time int(11) NOT NULL DEFAULT 0,
  PRIMARY KEY (category_id)
)
...
CREATE TABLE album (
  ...
  type varchar(50) NOT NULL DEFAULT 'img' COMMENT '类型',
...
CREATE TABLE adv_position (
  ...
  ap_height int(11) NOT NULL DEFAULT 0 COMMENT '广告位高度',
  ap_width int(11) NOT NULL DEFAULT 0 COMMENT '广告位宽度',
...
CREATE TABLE adv (
  ...
  adv_url text DEFAULT NULL COMMENT '广告链接',
...
(9, 1, 0, 'VERIFY', 2, '核销成功通知', '尊敬的会员，您的订单{orderno}，商家核销成功。', '{"orderno":"others"}', 0, 0, 0, 0),
(10, 1, 0, 'REGISTER_CODE', 1, '注册验证', '您的验证码为：{code}，该验证码 5 分钟内有效，请勿泄漏于他人！', '{"code":"valid_code"}', 0, 0, 0, 1596426737),
(11, 1, 0, 'REGISTER_SUCCESS', 2, '注册成功', '注册成功, 尊敬的{username},您已成功注册为{shopname}用户。', '{"username":"others","shopname":"others"}', 0, 0, 0, 1596426734),
...
(29, 1, 0, 'USER_BALANCE_CHANGE_NOTICE', 2, '会员余额变动通知', '尊敬的{username}，您的账户余额发生变动{change_money}元。当前账户不可提现余额：{balance}，可提现余额：{balance_money}。', '{"username":"others","balance":"amount","balance_money":"amount"}', 0, 0, 0, 1616229040),
...
(34, 1, 0, 'COMMISSION_GRANT', 2, '分销佣金发放通知', '您在{sitename}推荐的{level}用户{username}付款啦，推广奖励已到账，继续加油哦', '{"level":"others","sitename":"others","username":"others"}', 1, 2, 0, 0),
(35, 1, 0, 'BARGAIN_COMPLETE', 2, '砍价成功提醒', '您砍价的商品{sku_name}已经砍到底价啦，快去购买吧', '{"sku_name":"others"}', 0, 0, 0, 0),
(36, 1, 0, 'PINTUAN_COMPLETE', 2, '拼团成功提醒', '您拼团的商品{sku_name}已经拼团成功。', '{"sku_name":"others"}', 0, 0, 0, 0),
(37, 1, 0, 'PINTUAN_FAIL', 2, '拼团失败提醒', '您拼团的商品{sku_name}拼团失败了，重新发起试试吧。', '{"sku_name":"others"}', 0, 0, 0, 0),
(38, 1, 0, 'CASHIER_MEMBER_VERIFY_CODE', 1, '收银台会员支付验证', '您正在进行支付验证，您的验证码为：{code}，请不要将该验证码泄露给收银员之外的人', '{"code":"valid_code"}', 0, 0, 0, 0),
(39, 1, 0, 'OFFLINEPAY_WAIT_AUDIT', 2, '线下支付待审核', '您有一个新的线下支付待审核。支付内容：{order_name}，支付金额：{pay_money}，流水号：{out_trade_no}', '{\"order_name\":\"others\",\"pay_money\":\"amount\",\"out_trade_no\":\"other_number\"}', 0, 0, 0, 0),
(40, 1, 0, 'OFFLINEPAY_AUDIT_REFUSE', 2, '线下支付审核拒绝', '您有一个线下支付审核被拒绝了。支付内容：{order_name}，支付金额：{pay_money}，流水号：{out_trade_no}', '{\"order_name\":\"others\",\"pay_money\":\"amount\",\"out_trade_no\":\"other_number\"}', 0, 0, 0, 0),
(41, 1, 0, 'MESSAGE_LOCAL_WAIT_DELIVERY', 2, '同城配送待配送提醒', '您有一个新的待配送订单。支付内容：{order_name}，支付金额：{pay_money}，流水号：{out_trade_no},请及时处理！！', '{\"order_name\":\"others\",\"pay_money\":\"amount\",\"out_trade_no\":\"other_number\"}', 0, 0, 0, 1727230794);
...
(4, 'floor-style-4', '样式4', '{"goodsList":{"type":"goods","value":{"goods_ids":"","list":[]}},"leftImg":{"type":"img","value":{"url":"","link":""}},"bottomImg":{"type":"img","value":{"url":"","link":""}},"title":{"type":"text","value":{"text":"楼层名称","link":"","color":"#333333"}},"more":{"type":"text","value":{"text":"查看更多","link":"","color":"#999999"}}}',3);
...
(5, '', 'ORDER_DELIVERY', '订单发货通知', 1, '{"orderno":"订单号"}', '', '{"alisms":{"template_id":"","content":"\\u5c0a\\u656c\\u7684\\u4f1a\\u5458\\uff0c\\u60a8\\u7684\\u8ba2\\u5355\\u5df2\\u53d1\\u8d27\\uff0c\\u8ba2\\u5355\\u53f7${orderno}\\u3002","smssign":""}}', '', '{"template_id_short":"42984","template_id":"5MdP7MJ67tCNmdfk9b4TmHaY5iEPITNvWd4FhHIChfo","headtext":"","bottomtext":"","headtextcolor":"","bottomtextcolor":"","content":"订单编号：{{character_string2.DATA}}\\n商品名称：{{thing4.DATA}}\\n商品件数：{{number5.DATA}}\\n支付金额：{{amount8.DATA}}\\n发货时间：{{time12.DATA}}","keyword_name_list":["订单编号","商品名称","商品数量","订单金额","发货时间"]}', '{"tid":"30766","kidList":[2,1,7,3],"content":"\\u8ba2\\u5355\\u53f7{{character_string2.DATA}}\\n\\u5546\\u54c1\\u540d\\u79f0{{thing1.DATA}}\\n\\u8ba2\\u5355\\u91d1\\u989d{{amount7.DATA}}\\n\\u53d1\\u8d27\\u65f6\\u95f4{{date3.DATA}}","sceneDesc":"\\u8ba2\\u5355\\u53d1\\u8d27\\u901a\\u77e5"}', '', 'sms,wechat,weapp', '只针对实物商品物流发货，卖家发货完成后进行发送'),
(7, '', 'ORDER_REFUND_AGREE', '商家同意退款', 1, '{"orderno":"订单号"}', '', '{"alisms":{"template_id":"","content":"\\u5c0a\\u656c\\u7684\\u4f1a\\u5458\\uff0c\\u60a8\\u7684\\u8ba2\\u5355${orderno}\\uff0c\\u5546\\u5bb6\\u540c\\u610f\\u9000\\u6b3e\\u3002","smssign":""}}', '', '{"template_id_short":"48058","template_id":"0AFIBsNJAlm6h1YK5-7QHtKV8K-yuInbaU-7a5OiA2o","headtext":"","bottomtext":"","headtextcolor":"","bottomtextcolor":"","content":"订单编号：{{character_string5.DATA}}\\n退款金额：{{amount2.DATA}}\\n退款时间：{{time4.DATA}}","keyword_name_list":["订单编号","退款金额","退款时间"]}', '{"tid":"30825","kidList":[3,1,7],"content":"\\u8ba2\\u5355\\u7f16\\u53f7{{character_string3.DATA}}\\n\\u9000\\u6b3e\\u91d1\\u989d{{amount1.DATA}}\\n\\u7533\\u8bf7\\u7ed3\\u679c{{phrase7.DATA}}","sceneDesc":"\\u5546\\u5bb6\\u540c\\u610f\\u9000\\u6b3e"}', '', 'sms,wechat,weapp', '卖家同意退款后对买家发送'),
(8, '', 'ORDER_REFUND_REFUSE', '商家拒绝退款', 1, '{"orderno":"订单号"}', '', '{"alisms":{"template_id":"","content":"\\u5c0a\\u656c\\u7684\\u4f1a\\u5458\\uff0c\\u60a8\\u7684\\u8ba2\\u5355${orderno}\\uff0c\\u5546\\u5bb6\\u62d2\\u7edd\\u9000\\u6b3e\\u3002","smssign":""}}', '', '{"template_id_short":"49580","template_id":"CcbUfvEeZ05maqYbnuTqsNWPnx3V7pBCoDL42_JDUxE","headtext":"","bottomtext":"","headtextcolor":"","bottomtextcolor":"","content":"订单编号：{{character_string1.DATA}}\\n退款金额：{{amount2.DATA}}","keyword_name_list":["订单编号","退款金额"]}', '{"tid":"30824","kidList":[4,3],"content":"\\u8ba2\\u5355\\u53f7{{character_string4.DATA}}\\n\\u9000\\u6b3e\\u91d1\\u989d{{amount3.DATA}}","sceneDesc":"\\u5546\\u5bb6\\u540c\\u610f\\u9000\\u6b3e"}', '', 'sms,wechat,weapp', '卖家拒绝退款申请后对买家进行发送'),
(9, '', 'VERIFY', '核销成功通知', 1, '{"orderno":"订单号"}', '', '{"alisms":{"template_id":"","content":"\\u5c0a\\u656c\\u7684\\u4f1a\\u5458\\uff0c\\u60a8\\u7684\\u8ba2\\u5355${orderno}\\uff0c\\u5546\\u5bb6\\u6838\\u9500\\u6210\\u529f\\u3002","smssign":""}}', '', '{"template_id_short":"47357","template_id":"HMHlkbBDWH-z31kvAZ8306RcWFULADjknLNYW_zr4Jc","headtext":"你好，商品核销成功","bottomtext":"感谢您的使用,祝您生意兴隆。","headtextcolor":"","bottomtextcolor":"#c11010","content":"商品名称：{{thing2.DATA}}\\n核销数量：{{character_string6.DATA}}\\n核销时间：{{time4.DATA}}","keyword_name_list":["商品名称","核销数量","核销时间"]}', '{"tid":"31517","kidList":[1,2,3],"content":"\\u6838\\u9500\\u72b6\\u6001{{phrase1.DATA}}\\n\\u6838\\u9500\\u65f6\\u95f4{{thing2.DATA}}\\n\\u8ba2\\u5355\\u7f16\\u53f7{{character_string3.DATA}},","sceneDesc":"\\u6838\\u9500\\u7801\\u8fc7\\u671f\\u63d0\\u9192"}', '', 'sms,wechat,weapp', '核销码核销成功后对买家发送，可以是虚拟商品核销、门店自提订单核销'),
(10, '', 'BUYER_REFUND', '订单维权通知', 2, '{"username":"会员名称","goodsname":"商品名称","orderno":"订单号","refundmoney":"退款金额","refundreason":"退款原因","refundno":"退款单号"}', '', '{"alisms":{"template_id":"","content":"${username}\\u7533\\u8bf7\\u4e86\\u9000\\u6b3e\\uff0c\\u8ba2\\u5355\\u53f7\\u4e3a${orderno}\\u7684\\u5546\\u54c1${goodsname}\\uff0c\\u9000\\u6b3e\\u5355\\u53f7\\u4e3a${refundno}\\uff0c\\u9000\\u6b3e\\u91d1\\u989d${refundmoney}\\uff0c\\u9000\\u6b3e\\u539f\\u56e0${refundreason}\\u3002","smssign":""}}', '', '{"template_id_short":"49399","template_id":"7vnKqsbLvg7Y3p27Klj-ac6ubgQDVBF5TJ7e6rjkUIA","headtext":"","bottomtext":"","headtextcolor":"","bottomtextcolor":"","content":"订单编号：{{character_string3.DATA}}\\n申请时间：{{time10.DATA}}\\n商品名称：{{thing2.DATA}}\\n退款金额：{{amount1.DATA}}","keyword_name_list":["订单编号","申请时间","商品名称","退款金额"]}', '', '', 'sms,wechat', '买家提交退款申请后对卖家进行发送'),
(11, '', 'BUYER_DELIVERY_REFUND', '买家已退货通知', 2, '{"orderno":"订单号"}', '', '{"alisms":{"template_id":"","content":"\\u8ba2\\u5355\\u53f7\\u4e3a${orderno}\\uff0c\\u4e70\\u5bb6\\u9000\\u8d27\\u3002","smssign":""}}', '', '{"template_id_short":"46233","template_id":"m_xeaEObfOLjvI7Fii464EiCT34u1wHzI0zdE0s3NxA","headtext":"","bottomtext":"","headtextcolor":"","bottomtextcolor":"","content":"订单号：{{character_string6.DATA}}\\n商品名称：{{thing3.DATA}}\\n申请时间：{{time4.DATA}}","keyword_name_list":["订单号","商品名称","申请时间"]}', '', '', 'sms,wechat', '买家提交退货信息后对卖家进行发送'),
...
(20, '', 'BUYER_PAY', '买家支付通知', 2, '{"orderno":"订单号","ordermoney":"订单金额"}', '', '{"alisms":{"template_id":"","content":"\\u5c0a\\u656c\\u7684\\u5546\\u5bb6\\uff0c\\u4f1a\\u5458\\u8ba2\\u5355${orderno}\\u652f\\u4ed8\\u6210\\u529f\\uff0c\\u8ba2\\u5355\\u91d1\\u989d\\uff1a${ordermoney}\\u3002","smssign":""}}', '', '{"template_id_short":"47662","template_id":"MOSjtlQ3UVmbqOSEif1x8o3-vdYOFwmzfMsoF8TjUYM","headtext":"","bottomtext":"","headtextcolor":"","bottomtextcolor":"","content":"支付时间：{{time4.DATA}}\\n订单编号：{{character_string2.DATA}}\\n客户名称：{{thing1.DATA}}\\n产品名称：{{thing8.DATA}}\\n订单金额：{{amount3.DATA}}","keyword_name_list":["支付时间","订单编号","客户名称","产品名称","订单金额"]}', '', '', 'sms,wechat', '买家订单支付成功后对卖家发送'),
(21, '', 'USER_WITHDRAWAL_APPLY', '会员申请提现通知', 2, '{"username":"会员名称","money":"提现金额"}', '', '{"alisms":{"template_id":"","content":"\\u5c0a\\u656c\\u7684\\u5546\\u5bb6\\uff0c\\u4f1a\\u5458${username}\\u7533\\u8bf7\\u63d0\\u73b0\\uff0c\\u63d0\\u73b0\\u91d1\\u989d${money}\\u3002","smssign":""}}', '', '{"template_id_short":"42935","template_id":"JGmiq75JpNdeCbWY_QMUIIuMPc_yKwXhIAakPTL2BWw","headtext":"","bottomtext":"","headtextcolor":"","bottomtextcolor":"","content":"客户名称：{{thing3.DATA}}\\n提现金额：{{amount6.DATA}}\\n提现时间：{{time8.DATA}}","keyword_name_list":["客户名称","提现金额","提现时间"]}', '', '', 'sms,wechat', '买家申请余额提现后对卖家进行通知'),
(22, '', 'FENXIAO_WITHDRAWAL_APPLY', '分销申请提现通知', 2, '{"fenxiaoname":"分销商名称","money":"提现金额"}', '', '{"alisms":{"template_id":"","content":"\\u5c0a\\u656c\\u7684\\u5546\\u5bb6\\uff0c\\u5206\\u9500\\u5546${fenxiaoname}\\u7533\\u8bf7\\u63d0\\u73b0\\uff0c\\u63d0\\u73b0\\u91d1\\u989d${money}\\u3002","smssign":""}}', '', '{"template_id_short":"42935","template_id":"JGmiq75JpNdeCbWY_QMUIL7G3qPatLJmHmovtPpnTb4","headtext":"","bottomtext":"","headtextcolor":"","bottomtextcolor":"","content":"客户名称：{{thing3.DATA}}\\n提现金额：{{amount6.DATA}}\\n提现时间：{{time8.DATA}}","keyword_name_list":["客户名称","提现金额","提现时间"]}', '', '', 'sms,wechat', '买家发起佣金提现后对卖家进行通知'),
(23, '', 'USER_WITHDRAWAL_SUCCESS', '会员提现成功通知', 1, '{"username":"会员名称","money":"提现金额"}', '', '{"alisms":{"template_id":"","content":"\\u5c0a\\u656c\\u7684{username}\\uff0c\\u4f60\\u63d0\\u73b0{money}\\u6210\\u529f\\u3002","smssign":""}}', '', '{"template_id_short":"49581","template_id":"9sh4M9dOt0N3RUb17Pk7Fj-BjsDV2y84lDz7m3TVo64","headtext":"","bottomtext":"","headtextcolor":"","bottomtextcolor":"","content":"提现金额：{{amount1.DATA}}\\n提现日期：{{time3.DATA}}","keyword_name_list":["提现金额","提现日期"]}', '{"tid":"30790","kidList":[6,3],"content":"\\u63d0\\u73b0\\u91d1\\u989d{{amount6.DATA}}\\n\\u63d0\\u73b0\\u65f6\\u95f4{{date3.DATA}}","sceneDesc":"\\u4f1a\\u5458\\u63d0\\u73b0\\u6210\\u529f\\u901a\\u77e5"}', '', 'sms,wechat,weapp', '会员余额提现到账后进行发送'),
(24, '', 'FENXIAO_WITHDRAWAL_SUCCESS', '分销提现成功通知', 1, '{"fenxiaoname":"分销商名称","money":"提现金额"}', '', '{"alisms":{"template_id":"","content":"\\u5c0a\\u656c\\u7684\\u5206\\u9500\\u5546${fenxiaoname}\\uff0c${money}\\u63d0\\u73b0\\u6210\\u529f\\u3002","smssign":""}}', '', '{"template_id_short":"49581","template_id":"9sh4M9dOt0N3RUb17Pk7FiDtBEEESJBO-Fcn7fT9tLA","headtext":"","bottomtext":"","headtextcolor":"","bottomtextcolor":"","content":"提现金额：{{amount1.DATA}}\\n提现日期：{{time3.DATA}}","keyword_name_list":["提现金额","提现日期"]}', '{"tid":"30813","kidList":[1,2,3],"content":"\\u63d0\\u73b0\\u91d1\\u989d{{amount1.DATA}}\\n\\u63d0\\u73b0\\u65f6\\u95f4{{time2.DATA}}\\n\\u5907\\u6ce8{{thing3.DATA}}","sceneDesc":"\\u4f1a\\u5458\\u7533\\u8bf7\\u63d0\\u73b0\\u901a\\u77e5"}', '', 'sms,wechat,weapp', '会员佣金提现成功后进行发送'),
...
(27, '', 'USER_CANCEL_APPLY', '会员注销申请', 2, '{"username":"会员账号"}', '', '{"alisms":{"template_id":"","content":"\\u5c0a\\u656c\\u7684\\u5546\\u5bb6\\uff0c\\u4f1a\\u5458${username}\\u7533\\u8bf7\\u6ce8\\u9500\\u8d26\\u53f7\\u3002","smssign":""}}', '', '{"template_id_short":"43312","template_id":"ayBjfc6cejtiGDqnnV98AMdyepNgzk8ZinDi_qK9xYE","headtext":"","bottomtext":"","headtextcolor":"","bottomtextcolor":"","content":"申请人：{{thing2.DATA}}\\n注销时间：{{time4.DATA}}","keyword_name_list":["用户名","注销时间"]}', '', '', 'sms,wechat', '会员提交账户注销申请后对商家进行发送'),
(28, '', 'FENXIAO_WITHDRAWAL_ERROR', '分销提现失败通知', 1, '{"fenxiaoname":"分销商名称","money":"提现金额"}', '', '{"alisms":{"template_id":"","content":"\\u5c0a\\u656c\\u7684{username}\\uff0c\\u60a8\\u7533\\u8bf7\\u7684{money}\\u4f63\\u91d1\\uff0c\\u63d0\\u73b0\\u5931\\u8d25\\u3002","smssign":""}}', '', '', '{"tid":"31387","kidList":[2,4],"content":"\\u63d0\\u73b0\\u91d1\\u989d{{amount2.DATA}}\\n\\u63d0\\u73b0\\u8fdb\\u5ea6{{thing4.DATA}}","sceneDesc":"\\u5206\\u9500\\u63d0\\u73b0\\u5931\\u8d25\\u901a\\u77e5"}', '', 'sms,weapp', '会员佣金提现失败后进行发送'),
(29, '', 'USER_BALANCE_CHANGE_NOTICE', '会员余额变动通知', 1, '{"username":"会员名称","balance":"不可提现余额","balance_money":"可提现余额","change_money":"变动金额"}', '', '{"alisms":{"template_id":"","content":"尊敬的${username}，您的账户余额发生变动${change_money}元。当前账户不可提现余额：${balance}，可提现余额：${balance_money}。","smssign":""}}', '', '', '', '', 'sms', '会员余额发生收入、支出发生改变时发送'),
(30, '', 'ORDER_VERIFY_OUT_TIME', '核销商品临期提醒', 1, '{"username":"会员名称","sku_name":"商品名称","expire_time":"核销码到期时间"}', '', '{"alisms":{"template_id":"","content":"\\u5c0a\\u656c\\u7684${username}\\uff0c\\u60a8\\u7684\\u5546\\u54c1${sku_name}\\uff0c\\u8ba2\\u5355\\u7684\\u6838\\u9500\\u65f6\\u95f4\\u5c06\\u4e8e${expire_time}\\u5230\\u671f\\uff01","smssign":""}}', '', '', '{"tid":"30781","kidList":[1,2,3],"content":"\\u670d\\u52a1\\u540d\\u79f0{{thing1.DATA}}\\n\\u5230\\u671f\\u65f6\\u95f4{{date2.DATA}}\\n\\u6e29\\u99a8\\u63d0\\u793a{{thing3.DATA}}","sceneDesc":"\\u6838\\u9500\\u7801\\u4e34\\u671f\\u63d0\\u9192"}', '', 'sms,weapp', '核销码过期前一段时间（可设置时间）对买家发送'),
(32, '', 'ORDER_URGE_PAYMENT', '订单催付通知', 1, '{"goodsname":"商品名称","expiretime":"到期时间","url":"商品链接"}', '', '{"alisms":{"template_id":"","content":"\\u60a8\\u8d2d\\u4e70\\u7684\\u201c${goodsname}\\u201d\\u8fd8\\u6ca1\\u4ed8\\u6b3e\\u54e6\\uff0c\\u5e97\\u5bb6\\u5c06\\u4e3a\\u60a8\\u9884\\u7559\\u5230${expiretime}\\uff0c\\u518d\\u4e0d\\u4ed8\\u6b3e\\u5b9d\\u8d1d\\u5c31\\u88ab\\u522b\\u4eba\\u4e70\\u8d70\\u5566\\u3002\\u67e5\\u770b\\u8be6\\u60c5 ${url}","smssign":""}}', '', '', '{"tid":"30782","kidList":[1,2,3],"content":"\\u8BA2\\u5355\\u7F16\\u53F7{{character_string1.DATA}}\\n\\u8BA2\\u5355\\u5185\\u5BB9{{thing2.DATA}}\\n\\u652F\\u4ED8\\u91D1\\u989D{{character_string3.DATA}}","sceneDesc":"\\u8BA2\\u5355\\u50AC\\u4ED8\\u63D0\\u9192"}', '', 'sms,weapp', '未付款订单将会在订单关闭前10分钟对买家进行催付提醒'),
(33, '', 'VERIFY_CODE_EXPIRE', '核销码过期提醒', 1, '{"sitename":"站点名称","sku_name":"商品名称"}', '', '{"alisms":{"template_id":"","content":"\\u60a8\\u5728${sitename}\\u8d2d\\u4e70\\u7684${sku_name}\\u5df2\\u8fc7\\u671f\\uff01","smssign":""}}', '', '', '{"tid":"31934","kidList":[2,3,5,6],"content":"\\u8BA2\\u5355\\u7F16\\u53F7{{character_string6.DATA}}\\n\\u4E0B\\u5355\\u65F6\\u95F4{{time2.DATA}}\\n\\u8FC7\\u671F\\u65F6\\u95F4{{time3.DATA}}\\n\\u6E29\\u99A8\\u63D0\\u793A{{thing5.DATA}}","sceneDesc":"\\u6838\\u9500\\u7801\\u5230\\u671F\\u63D0\\u9192"}', '', 'sms,weapp', '核销码过期后对买家发送提醒'),
(35, '', 'COMMISSION_GRANT', '分销佣金发放通知', 1, '{"sitename":"站点名称","level":"级别","username":"用户名称"}', '', '{"alisms":{"template_id":"","content":"\\u60a8\\u5728${sitename}\\u63a8\\u8350\\u7684${level}\\u7528\\u6237[${username}]\\u4ed8\\u6b3e\\u5566\\uff0c\\u63a8\\u5e7f\\u5956\\u52b1\\u5df2\\u5230\\u8d26\\uff0c\\u7ee7\\u7eed\\u52a0\\u6cb9\\u54e6","smssign":""}}', '', '{"template_id_short":"49581","template_id":"9sh4M9dOt0N3RUb17Pk7FqS8v7N01t8HFa9zkVlUBMQ","headtext":"恭喜您，获得了一笔新的佣金，您可以在“我的佣金”中查看详细佣金信息。","bottomtext":"感谢您的使用,祝您生意兴隆。","headtextcolor":"","bottomtextcolor":"","content":"提现金额：{{amount1.DATA}}\\n提现日期：{{time3.DATA}}","keyword_name_list":["提现金额","提现日期"]}', '{"tid":"30948","kidList":[1,2,3],"content":"\\u8ba2\\u5355\\u91d1\\u989d{{amount1.DATA}}\\n\\u9884\\u4f30\\u4f63\\u91d1{{amount2.DATA}}\\n\\u5546\\u54c1\\u540d\\u79f0{{thing3.DATA}}\\n\\u65f6\\u95f4{{time4.DATA}}","sceneDesc":"\\u5206\\u9500\\u4f63\\u91d1\\u53d1\\u653e\\u901a\\u77e5"}', '', 'sms,wechat,weapp', '订单完成后对该笔订单的分销商发送'),
...
(37, 'bargain', 'BARGAIN_COMPLETE', '砍价成功通知', 1, '{"goodsname":"商品名称","price":"底价"}', '', '{"alisms":{"template_id":"asdf","content":"\\u60a8\\u5728${sitename}\\u63a8\\u8350\\u7684${level}\\u7528\\u6237[${username}]\\u4ed8\\u6b3e\\u5566\\uff0c\\u63a8\\u5e7f\\u5956\\u52b1\\u5df2\\u5230\\u8d26\\uff0c\\u7ee7\\u7eed\\u52a0\\u6cb9\\u54e6","smssign":""}}', '', '', '{"tid":"30871","kidList":[1,2,3,8],"content":"\\u5546\\u54C1\\u540D\\u79F0{{thing1.DATA}}\\n\\u91D1\\u989D{{amount2.DATA}}\\n\\u780D\\u4EF7\\u91D1\\u989D{{amount8.DATA}}\\n\\u5B8C\\u6210\\u65F6\\u95F4{{time3.DATA}}","sceneDesc":"\\u780D\\u4EF7\\u6210\\u529F\\u901A\\u77E5"}', '', 'sms,weapp', '砍价成功后通知会员'),
(38, 'pintuan', 'PINTUAN_COMPLETE', '拼团成功通知', 1, '{"goodsname":"商品名称","nickname":"团长"}', '', '{"alisms":{"template_id":"asdf","content":"\\u60a8\\u5728${sitename}\\u63a8\\u8350\\u7684${level}\\u7528\\u6237[${username}]\\u4ed8\\u6b3e\\u5566\\uff0c\\u63a8\\u5e7f\\u5956\\u52b1\\u5df2\\u5230\\u8d26\\uff0c\\u7ee7\\u7eed\\u52a0\\u6cb9\\u54e6","smssign":""}}', '', '', '{"tid":"30784","kidList":[5,7],"content":"\\u5546\\u54C1\\u540D\\u79F0{{thing5.DATA}}\\n\\u6210\\u56E2\\u65F6\\u95F4{{time7.DATA}}","sceneDesc":"\\u62FC\\u56E2\\u6210\\u529F\\u901A\\u77E5"}', '', 'sms,weapp', '拼团成功后通知会员'),
(39, 'pintuan', 'PINTUAN_FAIL', '拼团失败通知', 1, '{"goodsname":"商品名称","nickname":"团长","price":"商品金额"}', '', '{"alisms":{"template_id":"asdf","content":"\\u60a8\\u5728${sitename}\\u63a8\\u8350\\u7684${level}\\u7528\\u6237[${username}]\\u4ed8\\u6b3e\\u5566\\uff0c\\u63a8\\u5e7f\\u5956\\u52b1\\u5df2\\u5230\\u8d26\\uff0c\\u7ee7\\u7eed\\u52a0\\u6cb9\\u54e6","smssign":""}}', '', '', '{"tid":"30801","kidList":[1,3],"content":"\\u5546\\u54C1\\u540D\\u79F0{{thing1.DATA}}\\n\\u9000\\u6B3E\\u91D1\\u989D{{amount3.DATA}}","sceneDesc":"\\u62FC\\u56E2\\u5931\\u8D25\\u901A\\u77E5"}', '', 'sms,weapp', '拼团失败后通知会员'),
(40, 'cashier', 'CASHIER_MEMBER_VERIFY_CODE', '收银台会员码支付验证', 1, '{"code":"验证码"}', '', '', '', '', '', '', 'sms', '收银台进行会员码验证时发送'),
(41, '', 'OFFLINEPAY_WAIT_AUDIT', '线下支付待审核', 2, '{\"order_name\":\"订单名称\",\"pay_money\":\"支付金额\",\"out_trade_no\":\"外部交易号\"}', '', '{\"alisms\":{\"template_id\":\"\",\"content\":\"您有一个新的线下支付待审核。支付内容：${order_name}，支付金额：${pay_money}，流水号：${out_trade_no}\",\"smssign\":\"\"}}', '', '{\"template_id_short\":\"54183\",\"template_id\":\"\",\"headtext\":\"线下支付待审核\",\"bottomtext\":\"\",\"headtextcolor\":\"\",\"bottomtextcolor\":\"\",\"content\":\"单据名称：{{thing10.DATA}}\\n付款金额：{{amount4.DATA}}\\n流水号：{{character_string1.DATA}}\",\"keyword_name_list\":[\"单据名称\",\"付款金额\",\"流水号\"]}', '', '', 'sms,wechat', '买家提交线下支付凭证后提醒管理员及时审核单据'),
(42, '', 'OFFLINEPAY_AUDIT_REFUSE', '线下支付审核拒绝', 1, '{\"order_name\":\"订单名称\",\"pay_money\":\"支付金额\",\"out_trade_no\":\"外部交易号\"}', '', '{\"alisms\":{\"template_id\":\"\",\"content\":\"您有一个线下支付审核被拒绝了。支付内容：${order_name}，支付金额：${pay_money}，流水号：${out_trade_no}\",\"smssign\":\"\"}}', '', '{\"template_id_short\":\"49003\",\"template_id\":\"\",\"headtext\":\"订单审核驳回提醒\",\"bottomtext\":\"\",\"headtextcolor\":\"\",\"bottomtextcolor\":\"\",\"content\":\"订单名称：{{thing10.DATA}}\\n订单金额：{{amount6.DATA}}\\n订单编号：{{character_string5.DATA}}\",\"keyword_name_list\":[\"订单名称\",\"订单金额\",\"订单编号\"]}', '{\"tid\":\"30849\",\"kidList\":[2,3,1,5],\"content\":\"商品名称{{thing2.DATA}}\\n订单金额{{amount3.DATA}}\\n订单编号{{character_string1.DATA}}\\n失败原因{{thing5.DATA}}\",\"sceneDesc\":\"线下支付审核拒绝后通知买家\"}', '', 'sms,wechat,weapp', '线下支付凭证审核拒绝通知买家'),
(43, '', 'MESSAGE_LOCAL_WAIT_DELIVERY', '同城配送待配送提醒', 2, '{\"order_name\":\"订单名称\",\"pay_money\":\"支付金额\",\"out_trade_no\":\"外部交易号\"}', '', '{\"alisms\":{\"template_id\":\"\",\"content\":\"您有一个新的待待配送订单。支付内容：${order_name}，支付金额：${pay_money}，流水号：${out_trade_no},请及时处理!!\",\"smssign\":\"\"}}', '', '', '', '', 'sms', '同城订单指定配送员后提醒。');
...
(429004, 420000, '仙桃市', '仙桃', '113.45397', '30.364952', 3, 0, 1),
(429005, 420000, '潜江市', '潜江', '112.896866', '30.421215', 3, 0, 1),
(429006, 420000, '天门市', '天门', '113.16586', '30.65306', 3, 0, 1),
(429021, 420000, '神农架林区', '神农架', '114.29857', '30.584354', 3, 0, 1),
...
(659000, 650000, '直辖县级行政区', '直辖县级行政区', '', '', 2, 0, 1),
...
(441900001, 441900, '东莞市', '东莞市', '114.051881', '22.913189', 3, 0, 1),
...
(442000000, 442000, '中山市', '中山市', '113.392782', '22.517645', 3, 0, 1),
...
(20, 1, '卡项专区', '卡项', 0, 0, '', '#ffffff', 2, 'NS_CARD', 0, 1),
(21, 1, '品牌专区', '品牌', 0, 0, '', '#ffffff', 2, 'NS_BRAND', 0, 1);
...
(21, 1, 20, '广告一', '', 'public/static/img/diy_view/index_card.png', 0, 0.00, '#FFFFFF', 1);

INSERT INTO cashier_auth_group(group_name, menu_array, keyword, site_id, `desc`) VALUES
('门店管理员', '', 'admin', 1, ''),
('收银员', 'cashier,billing,buycard,recharge', '', 1, ''),
('核销员', 'verify_manage,verify_index,verify_record,verify_code_info,verify', '', 1, '');

INSERT INTO store(store_name,site_id,status,stock_type,is_default) values('默认总店',1,1,'store', 1);

INSERT INTO cron(type, period, period_type, name, event, execute_time, relate_id, create_time) VALUES
(2, 2, 0, '店铺统计更新(按时)', 'CronStatShopHour', 1700447491, 0, 0),
(2, 2, 0, '门店统计更新(按时)', 'CronStatStoreHour', 1700447491, 0, 0),
(2, 2, 0, '店铺统计更新(按日)', 'CronStatShop', 1700447491, 0, 0),
(2, 2, 0, '门店统计更新(按日)', 'CronStatStore', 1700447491, 0, 0);

CREATE TABLE stat_shop_cron  (
                                             id int NOT NULL AUTO_INCREMENT,
