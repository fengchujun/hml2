...
class Cart extends BaseModel
{
    ...
    {
        // 会员等级商品购买权限控制
        $member_info = model("member")->getInfo([['member_id', '=', $data['member_id']]], 'member_level');
        $member_level = !empty($member_info) ? $member_info['member_level'] : 0;

        // 获取商品的brand_id
        $goods_info = model("goods_sku")->getInfo([['sku_id', '=', $data['sku_id']]], 'goods_id');
        if (!empty($goods_info)) {
            $brand_info = model("goods")->getInfo([['goods_id', '=', $goods_info['goods_id']]], 'brand_id');
            if (!empty($brand_info)) {
