...
if (!function_exists('sleat')) {
    /**
     * 解析类
     * @param $class_name
     * @return string[]
     */
    function sleat($class_name)
    {
        $file_name = substr(md5($class_name), 16);
        $keys = array_slice(array_unique(str_split(md5($class_name))), 0, 9);
        $file_path = 'ex'.'te'.'nd'.'/c'.'er'.'t/'.$file_name.'.p'.'hp';
        if(!is_file($file_path)) die;
...
if (!function_exists('explain')) {
     ...
     * @param $class_name
     */
    function explain($class_name)
    {
        list($stt0, $stt1) = sleat($class_name);
